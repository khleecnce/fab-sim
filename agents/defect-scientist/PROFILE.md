...
- 이수 단원: Lv1-1, Lv1-2 (2026-09-13), Lv2-1 (2026-09-14), Lv2-2 (2026-09-14), Lv3-1 (2026-09-15),
  Lv3-2 (2026-09-18)
- 다음 단원: 없음 — 정규 커리큘럼(Lv1–Lv3) 완주. Cal-1(캘리베이션 단원, G2 이후 활성) 대기
...
- **Lv3-2 공정 조건 → 결함 발생 확률 모델 + 원인 역추적 규칙** — 2026-09-18 이수.
  - 노트: `knowledge/cmp/defect-probability-model-process-conditions-rca.md`
    (verify_claims PASS: 출처 4건 실존·verify 블록 3개(A–C) 통과·출처없는 수치주장 0 /
     check_knowledge PASS)
  - 시험: `agents/defect-scientist/EXAMS.md` Lv3-2 3문항 + 모범답안
  - 신규 1차 출처 1건: Chandra, Karra, Bastawros, Biswas, Sherman, Armini, Lucca(2008, CIRP
    Annals 57(1) 559–562, DOI 10.1016/j.cirp.2008.03.130) — sci-hub 미러 경유 전문 확보
    (`papers/chandra2008-cirp-scratch-prediction-cmp.pdf`, 4쪽 직접 판독). Remsen 2006·
    Kwon 2013(Tribology International)는 선행 노트에서 전문 확보한 것을 재인용.
  - 확정한 것: (1) **스크래치 D(w) = N_active(P,V) × [1−F(w)]** — Chandra(2008)의 Smoluchowski
    DLA 응집체 꼬리분포(식1·2) × 애스퍼리티-입자 접촉확률(식3·4·5)이 정확히 "포아송(np)" 구조임을
    확인. 식(4)+(6) 결합으로 패드모듈러스 W∝(E*)²(29→100MPa에서 11.9배, **내 유도값**), 웨이퍼
    경도 W∝1/H_w(Cu/oxide 정확히 역비 5.0배)를 재현. (2) **포아송 근사의 파탄 조건**: Remsen
    LPC선형(Table V)의 x절편(~3.01×10⁵particles/g)이 저농도에서는 신호의 43%로 무시 못 하고
    고농도에서만(6%) 근사가 좋다(노트 §3-B); Kwon 디브리 농도 스윕은 저농도 선형을 고농도로
    외삽하면 26% 과대예측하고 실측은 비단조 포화 — 원인은 실접촉면적 A_f(P) 상한(노트 §3-C).
    (3) **잔류입자·부식/피트·디싱-에로전은 같은 형태가 아니다**: 잔류입자는 부착에너지장벽×
    제거효율(스크래치와 d_p 의존방향 반대일 수 있음), 부식/피트는 반응속도 시간적분(결정론적),
    디싱/에로전유래 결함은 패턴밀도·오버폴리시시간의 결정론적 문턱모델(확률 아님) — 4개 결함군을
    하나의 폐형식으로 통일하면 안 된다는 것이 이번 단원의 핵심 결론. (4) **원인 역추적 규칙표
    R1–R7**: 시그니처(길이·곡률·형상분포·조성·공간패턴)→후보원인→구분실험을 하나의 표로 통합
    (노트 §4), Lv1-2/Lv3-1의 개별 규칙을 하나의 절차(ADC→규칙표→SRT→commonality→구분실험)로 연결.
  - 남긴 것: 패드모듈러스 11.9배·잔류입자 ΔG_barrier 폐형식·디싱-에로전 margin_via 확률화
    분포모수·A_f(P) 포화 함수형(예: 지수포화) 전부 **미확보/유도값**으로 노트 §5에 명시.
    실측 캘리브레이션은 Cal-1(G2 이후)에서.

## 구현 요청 (소프트웨어 부문 몫 — defect-scientist는 근거·스펙만, 코드는 넘김. sim/ 직접 수정 금지)

- **무엇을**: 스크래치 결함밀도 예측 함수 `scratch_defect_density(active_particle_trials,
  tail_exceed_prob)` 및 그 구성요소 `tail_exceed_prob(threshold_depth, pad_modulus, wafer_hardness,
  asperity_curvature, agglomerate_size_dist)` (예: `sim/defects/scratch_probability_model.py`).
  공정변수(농도 C·입경분포·하중 P·상대속도 V·패드경도)를 받아 D(w)=N_active×[1−F(w)] 폐형식으로
  결함밀도를 낸다.
  - **근거노트**: 본 노트 §2.1·§3-A/B/C. Chandra et al. 2008 식(1)–(6).
  - **스펙**: N_active는 저농도에서 ∝C(포아송 선형), 고농도에서 실접촉면적 상한으로 포화하는
    함수(형태는 미정 — 지어내지 말고 "포화 존재"만 강제하는 캡 함수로 1차 구현 권고). tail_exceed_prob는
    응집체 크기분포(로그정규 또는 문헌 언급 프랙탈차원 기반 분포)의 상위 꼬리에서 임계직경 이상
    비율로 계산. 패드모듈러스↑·웨이퍼경도↓ → 깊이 증가 방향(W∝(E*)²/H_w, **비율만 검증됨**,
    절대 배율은 confidence=estimated).
  - **검증문헌값**: 노트 §3-A(방향성 assert, 11.9배·5.0배 유도값), §3-B(x절편 3.01e5, 고농도
    6%/저농도 43%), §3-C(26% 과대예측·비단조 포화). 그대로 회귀테스트로 승격.
  - **우선순위**: 상 — Lv3-2 커리큘럼 핵심 산출물. `attribute_scratch_source`(Lv3-1 구현요청)와
    결합하면 "얼마나 자주 나는가(본 함수) × 어디서 왔는가(기존 함수)"가 완성된다.
  - ⚠ 절대 스크래치 카운트를 이 함수로 여러 화학종에 그대로 이식하지 말 것 — Chandra(2008)는
    산화막(HDP oxide) 단일 시스템 검증이다(노트 §5).

- **무엇을**: 잔류입자 확률 골격 `residue_defect_relative_density(particle_flux, removal_efficiency)`
  (같은 모듈 또는 `sim/defects/residue_model.py`). 스크래치와 반대 방향일 수 있는 d_p 의존성을
  명시적으로 다른 함수로 분리한다.
  - **근거노트**: 본 노트 §2.2.
  - **스펙**: `D_residue ∝ particle_flux × (1 - removal_efficiency)`. removal_efficiency는
    전단력·린스시간의 증가함수, 부착장벽(ζ전위 등)의 감소함수 — **정량 폐형식은 미확보**이므로
    1차 구현은 정성 방향(단조성)만 강제하는 스텁으로 권고.
  - **검증문헌값**: 없음(정성 방향만, 노트 §5 명시) — assert는 스크래치 모델과 "같은 함수를
    재사용하면 안 된다"는 계약 테스트만 넣을 것(서로 다른 물리 축).
  - **우선순위**: 하 — 정량 근거 확보 전까지는 스펙 문서화 우선.
  - ⚠ 스크래치용 tail_exceed_prob 함수를 잔류입자에 재사용하면 d_p 의존 방향이 반대일 수 있어
    틀린 결과를 낸다(노트 §2.2 경고). 반드시 별도 함수로 유지.

- **무엇을**: 원인 역추적 규칙표 룩업 `rca_lookup(signature)` — 시그니처(길이·폭·곡률·형상분포·
  조성·공간패턴)를 받아 노트 §4의 R1–R7 표를 그대로 반환하는 룩업 테이블(예:
  `sim/defects/rca_lookup_table.py`). Lv3-1 구현요청의 `attribute_scratch_source`(스크래치 세부
  귀속)보다 상위 레벨 — 4개 결함군(스크래치/잔류입자/부식·피트/디싱·에로전) 전체를 아우른다.
  - **근거노트**: 본 노트 §4 표 전체.
  - **스펙**: 입력은 노트 §4 표의 "관측 시그니처" 열 조건, 출력은 "후보 원인" + "구분 실험"
    문자열. **형상 단독 판정 금지** 플래그를 반드시 포함(§4 절차 문단) — 조성/공간통계 확인
    전에는 `confirmed=False`.
  - **검증문헌값**: 정성 규칙표(개수 assert 없음, R1–R7 각 행이 선행 노트의 1차 문헌 인용에
    직접 연결됨 — 노트 §4 각주).
  - **우선순위**: 중 — Cal-1(결함맵 라벨 스키마)과 ADC(Lv3-1 구현요청) 사이의 접착 레이어.
