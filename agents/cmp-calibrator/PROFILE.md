...
- 2026-09-18 Lv3-1 드리프트 감지(소모품 로트·패드 교체·툴 PM 후 재보정 트리거) 이수.
  노트: `knowledge/cmp/calibration-drift-detection-recalibration-triggers.md`.
  1차 출처 2건 원문 전문 확보·직접 판독(Hsu & Chang 2026, DOI 10.3390/pr14172714 — MDPI
  오픈액세스 전문 15쪽; Son & Lee 2021, DOI 10.3390/app11083521 — MDPI 오픈액세스 전문 15쪽),
  Polunchenko, Sokolov & Tartakovsky 2014(arXiv:1307.7126, Sri Lankan J. Applied Statistics
  15(2))는 전문 27쪽 판독해 EWMA 관리도 ARL 폐형식(식 3.8)과 Table 4.1/4.2 수치를 직접
  재현. Lucas & Saccucci (1990, Technometrics 32:1–12, DOI 10.1080/00401706.1990.10484583)는
  EWMA 관리도의 원조 ARL 문헌으로 위 arXiv 논문의 참고문헌 목록에서만 확인(원문 유료·미러
  실패, **2차 인용**). NIST/SEMATECH e-Handbook(공개 정부문서)은 EWMA/CUSUM 정의·설계식
  출처로만 인용. 슬러리 로트 변동·캐리어 멤브레인/리테이너링 PM 두 원인축은 **1차 정량 문헌
  확보 실패**로 노트에 정직하게 표시(⚠). check_knowledge.py·verify_claims.py 둘 다 통과.
  verify 블록 2개 실제 실행·통과: (1) PST2014 식(3.8)의 EWMA-지수 관리도 ARL 폐형식을 직접
  구현해 Table 4.1(θ=0.5, z=0)의 (λ,A,γ) 세 쌍(0.275/2.07/100, 0.096/1.79/1000,
  0.049/1.67/10000)을 1.5% 이내로 재현; (2) 같은 파라미터로 실제 EWMA 관리도를 몬테카를로
  시뮬레이션해 3% 이내로 독립 재현(폐형식 유도 오류 배제).
  핵심 결론: (1) CMP 드리프트는 성격이 다른 두 트리거로 나뉜다 — 잔차 EWMA/CUSUM 기반
  사후 탐지(HC2026의 R2R EWMA 추정기가 쓰는 것과 같은 재귀식이지만 용도는 관리도)와, 패드
  교체 같은 소모품 이벤트에서의 결정론적 리셋(HC2026 §4.4가 실증 — 통계 판정 없이 리셋해도
  5개 패드 평균 제거율 95% CI가 [1696.16, 1699.61]로 일관). (2) 관리도 민감도(λ)와 오경보율
  (ARL₀=γ)은 같은 손잡이로 움직이는 트레이드오프이며, PST2014 Table 4.1이 이를 정량화한다
  (γ 10배 ⇒ λ 약 1/3, SADD 약 2.6배 증가) — 단 이 절대수치는 지수분포 모형 것이라 CMP
  잔차(정규분포에 가까움)에 그대로 이식하는 것은 미검증, 트레이드오프 방향성만 확실.
  (3) SL2021의 패드 컨디셔닝 실측(12시간 변화점, 16시간 44.9% MRR 감소)은 "선형 드리프트가
  아니라 변화점을 갖는 궤적"의 실제 CMP 사례로, EWMA/CUSUM 변화점 검출이 겨냥해야 할
  신호 형태를 구체적으로 보여준다. (4) FabSim 축 매핑: 패드마모→S, 디스크마모→Γ,
  슬러리로트→Kp는 `sim/factors.py` 설계와 정합되지만, 캐리어 멤브레인/리테이너링 PM은
...
### 드리프트 감지 + 재보정 트리거 API (Lv3-1 산출, 근거노트:
`knowledge/cmp/calibration-drift-detection-recalibration-triggers.md`)

무엇을: series_scale/GP 보정치가 언제 무효화됐는지 감시하는 잔차 관리도 + 이벤트 기반 리셋
두 트리거를 분리 구현한다.

- `residual_ewma_signal(residual_log_ratio, lam, L) -> dict(z_t, ucl, lcl, flagged)`
  - 입력: 시계열로 들어오는 잔차(실측/예측 MRR 비의 로그), EWMA 가중치 λ, 관리한계 배수 L.
  - 출력: NIST식 `UCL/LCL = z0 ± L·σ·sqrt(λ/(2-λ))`(근거노트 §2.1)로 계산한 관리한계와
    현재 통계량, 위반 여부.
  - 검증문헌값(근거노트 verify(1)(2)): PST2014 Table 4.1의 (λ,A,γ) 설계쌍이 실제로 그
    오경보 ARL을 내는지 폐형식·몬테카를로 둘 다로 확인됨(1~3% 이내). λ를 줄이면 γ(오경보
    ARL)는 늘고 탐지지연도 늘어난다는 트레이드오프가 있으므로, **구현 시 λ·L을 하드코딩
    상수로 박지 말고 "원하는 오경보 ARL"을 입력받아 역산하는 인터페이스로 설계할 것**
    (지수분포 모형의 절대수치는 CMP 잔차에 그대로 쓰면 안 됨 — 방향성만 신뢰, 근거노트 §2.2).
  - 우선순위: 높음(series_scale의 배율이 조용히 틀어지는 것을 잡는 유일한 통로).

- `consumable_reset_event(event_type, resets=["ewma_state","knn_db","series_scale_fit"]) -> None`
  - 입력: 패드 교체·슬러리 로트 변경·디스크 교체·캐리어 멤브레인/리테이너링 교체 이벤트 로그.
  - 동작: 통계 판정 없이 잔차 이력·EWMA 상태를 초기화하고 재보정 런을 요구(근거노트 §3
    규칙 C, HC2026 §4.4가 실증 — 5개 패드 무조건 리셋 후에도 평균 제거율 95% CI가
    [1696.16, 1699.61]로 일관).
  - 우선순위: 높음(구현 비용이 낮고 근거가 가장 확실한 트리거).
