...
## Lv3-2 CMP 마찰계수 실측·레짐·Preston 상수 붕괴 (2026-09-08)
근거: [[../../knowledge/physics/cmp-friction-regime-experimental-mit-lai]], [[../../knowledge/physics/cmp-lubrication-regimes]], [[../../knowledge/physics/preston-luo-dornfeld-mrr]]

**Q1. Lai(2001) 실측에서 저 ηv_R/p 영역의 COF(0.40~0.49)가 압력·속도에 무관한 이유를 Coulomb 마찰로 설명하고, 이 값이 Lv1-2([[cmp-lubrication-regimes]])의 oxide CMP 문헌값(0.23~0.40)과 다른 이유를 재료 관점에서 논하라.**
A1. Coulomb 마찰은 F_t=μF_n에서 μ가 하중·속도와 무관한 상수로 정의된다. 접촉모드(저 ηv_R/p, 즉
저속·고압)에서는 패드 asperity가 웨이퍼와 고체접촉하고 그 접촉면적이 하중에 선형 비례(Hertz/GW 모델의
탄소성 극한)하므로 μ가 상수로 남는다 — 실측(Fig 2.5)이 이를 직접 확인한다. 값이 oxide(0.23~0.40, 2차
인용)보다 높은 것은 이 실험이 **Cu CMP**(중성 pH 7, Al₂O₃ 슬러리)이기 때문 — 재료 경도·표면화학이
다르면 실접촉면적과 전단강도가 달라져 μ_boundary가 재료별로 분화된다. 기존 `cof_stribeck(mu_bl=0.30)`은
oxide 기준값이라 Cu 접촉모드를 과소평가한다(본 노트 §6 검증코드로 확인, 0.30 < 0.40).

**Q2. Lai(2001)가 "CMP는 실험적으로 hydroplaning에 도달한 적이 없다"고 결론짓는 근거 3가지를 요약하고, 그중 어느 것이 Lv1-2의 λ≥3 경계와 독립적으로 일치하는가?**
A2. (1) 2차원 slurry flow의 side-leakage로 1차원 Reynolds 해석이 실제 유량을 과소평가, (2) 압력중심이
항상 출구쪽(y_cp>0.5D)에 치우쳐 중심 짐벌설계와 어긋나 받침각이 붕괴, (3) hydroplaning 성립 조건이
"필름두께가 복합 RMS 거칠기의 3배 이상"인데 실무 패드 waviness(패드 파상도)가 이를 만족 못함. 이 중
(3)의 "3배" 수치가 Lv1-2에서 일반 트라이볼로지 관례(Bhushan 2013, tribonet.org 2차 인용)로만 확정했던
λ≥3 hydrodynamic 경계와 **독립적인 CMP 전용 1차원 유체역학 해석에서 같은 배수로 재도출**된다 — 두
경로가 수렴했다는 점에서 λ=3 경계의 신뢰도가 올라간다(본 노트 §4, §6 코드로 확인).

**Q3. Fig 2.8에서 Preston 상수 k_p가 14kPa에서 (ηv_R/p)^(-1), 48kPa에서 (ηv_R/p)^(-0.5)로 다른 지수를 보인다. 이것이 [[preston-luo-dornfeld-mrr]]의 "Kp=상수" 가정에 주는 함의는? 그리고 원인이 완전히 설명되었는가?**
A3. 접촉모드에서는 k_p가 압력·속도 무관 상수이지만, mixed 모드 진입 후에는 압력별로 **지수 자체가
다른 멱법칙**을 따른다 — 이는 Preston 식의 "Kp=상수"가 boundary 접촉이 유지되는 좁은 조건에서만
성립하는 근사이며, 레짐이 바뀌면 Kp가 압력에 의존적인 함수로 붕괴함을 실측이 직접 보여준다. 원인은
§2.2.5의 혼합모드 마찰모델(접촉면적비 α,β가 ηv_R/p에 따라 변함)로 정성적으로만 설명되고, **왜 지수가
압력에 따라 -1과 -0.5로 갈리는지는 원문에도 명시적 유도가 없다 — 미검증(원인 미상)**으로 남긴다.
