...
## 이수 기록 (계속 2)
- Δ/κ `abrasive_d99_nm` 1차출처 재탐색 (2026-09-19): K-안정 콜로이달 실리카(DuPont/Versum/
  Fuso/Nalco) 자체의 D99 실측은 이번에도 미확보 — 값·등급(103.824688nm, estimated) 불변.
  대신 US11,725,116 B2(CMC Materials) Table 1C에서 D50=37~127nm 구간 23개 조성의 disc
  centrifuge D10/D50/D90 실측표를 확보, D50=55nm 일치점(1F)에서 Hatch-Choate 외삽 D99≈70.4nm
  (현재값과 47% 괴리, 화학·용도 불일치로 미채택) 및 D99/D50 비율의 크기의존성 정량치(r≈-0.59,
  r²≈0.35 — 완전 불변도 완전 크기결정도 아님)를 새로 확보. 근거 노트:
  knowledge/cmp/abrasive-d99-psd-width-size-dependence.md (게이트 통과). 다음 회차 후보:
  US9422456B2/US9422457B2(WebFetch 3회 ECONNRESET, 미확보) — §6 참조.
