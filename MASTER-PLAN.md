...
- 2026-09-18 [Max워커] **판정#59·#60 — 정체성 상속 결함 수정 + 상시감사 도구. ⚠ 격자 60/60 → 59/60(의도된 후퇴).**
  (1) **판정#59**: `sic_alumina_kmno4` 의 `_f_chi` 분기 판정 키 `abrasive` 가 부모(`sic_ceria_h2o2`)의
  `'ceria'` 를 own 없이 상속 중이었다 — 같은 파일의 α-Al2O3 밀도 3950·description·출처 2건
  (US20220315802A1, Gong 2024 doi:10.3390/ma17030679)이 전부 알루미나인데 분기 키만 세리아였다.
  결과: **알루미나 슬러리에 세리아 chemical tooth(Ce³⁺ 분율)가 켜져 있었고** pH 분기도 세리아 IEP
  창으로 들어가 죽어 있었다. 선행 노트의 진단("`wafer_iep_ph` 부재")은 **오진**이며 그대로 따랐다면
  알루미나 계에 세리아 정전창을 씌우는 오염이 발생했을 것이다. ⚠ 순진한 수정도 함정 — `abrasive:
  alumina` 만 선언하면 폴백이 `ph_peak`(oxide_silica 소유, 정점 pH=11 실리카 전용)로 떨어져 실리카
  곡선을 산성 알루미나계에 씌운다(실측 확인). 팩 이름 하드코딩 없이 일반 규칙으로 차단: "고유 계수를
  상속만 받았고 그 계수를 **소유한 조상 팩**의 `abrasive` 가 이 팩과 다르면 그 분기를 쓰지 않는다"
  (판정#34 원칙), 막히면 어느 키가 어느 재료에서 왔는지 notes 에 명시. **격자 60/60 → 59/60
  (C1 χ/sic_alumina_kmno4: unmodeled) — 되돌리지 않는다**: 이전의 `modeled` 는 "모델링돼 있었다"가
  아니라 **"틀린 것이 모델링돼 있었다"** 이고, 격자가 내려간 게 아니라 **그동안 1칸 과대계상돼 있었다**
  (COMPLETION.md "지어내 채우는 것은 완성이 아니라 오염"). MRR 농도축 비트 불변, 5팩 χ 전부 불변
  (수정 전 git stash 스냅샷 직접 대조). held-out 갭은 남지만 **이유가 바뀌었다**: "세리아 분기 skip"
  (틀린 이유) → "알루미나계 pH 계수 미확보"(맞는 이유). 커밋 a45bf10.
  (2) **판정#60**: 같은 유형이 판정#34·#48·#52·#59 로 **네 번** 반복됐고 전부 사람이 우연히 발견했다 —
  `tools/audit_identity_inheritance.py` 로 기계 상시신고 전환. 상속 파라미터를 **재료축**으로 분류해
  소유 조상과 대조한다. **축 분류가 핵심**: 축 없이 대조하면 64건에 `pad_ra_m`(패드 거칠기)이
  "연마입자가 다르다"고 섞이고(오탐이 섞이면 도구가 죽는다), 축을 넣으면 **18건**으로 줄어 전부 실제
  재료축만 남는다. 축 모르는 키는 추측 신고하지 않고 `--unclassified` 로 노출. 18건 섭동 결과
  **살아 있는 건 2건뿐**이고 15건은 판정#59·#50 이 **이미 막아둔 것** — 앞선 판정 효과의 독립 확인.
  **신규 결함**: `abrasive_size_nm`=120 nm(세리아 소유, literature) vs Gong 2024 알루미나 **500 nm**
  = **4.17배 불일치**, 정점 163 nm 양쪽 반대 가지. **값만 고치지 않았다** — 정점·지수가 전부 세리아
  형상이라 입경만 바꾸면 알루미나 입경을 세리아 곡선에 대입하는 꼴(판정#59 §2 와 동형 함정).
  코드·YAML 0줄 변경. 커밋 8fbe722.
  검증(Max워커 직접 실행): pytest **1073 passed**(1069→+4, 회귀 0) + 판정#60 계약 6건,
  qa_loop #258 --strict PASS, verify_claims ✓ check_knowledge ✓ (양 노트).
  ⚠ 위임세션은 판정#59 노트 §2 까지 쓰고 max-turns 소진 사망(로그 0바이트) → 나머지 전부 Max워커 직접 수행.
  ⚠ **verify assert 가 Max워커 자신의 산술 오류를 적발**(모델 변화폭 -49.3% 로 적었으나 실제 -47.97%) —
  "서술이 아니라 assert 가 증거"라는 원칙이 실제로 작동했다.
  **다음 회차 후보**: ㊄ 알루미나계 pH 문헌 확보(격자 59/60 복구의 유일 경로) · ㊅ 입경축 5키 정합 ·
  ㊆ 감사 사각지대 축소.

- 2026-09-18 [Max워커] **모델링 완성 판정 달성 — 격자 60/60**. (1) EVIDENCE-RULES 판정#56-종결:
  마지막 C2 미충족 칸(χ/sic_ceria_h2o2 `oxidizer_langmuir_K`=0.7600)을 3회차 규칙으로 영구
  종결. Song 2025(doi:10.1016/j.ceramint.2025.07.097)가 4중 색인(Unpaywall·OpenAlex·
  Semantic Scholar·CORE) 전부 셀프아카이브 사본 없음(2025-10 출간)으로 확인돼 값·등급
  불변, `validation/C2-CLOSURES.yaml` 등록으로 '검증된 한계'(12칸) 처리. (2) C8
  MODEL-BASIS.md 재생성 + 요약절의 하드코딩된 미충족 문장 제거(완성 판정과 문서가
  모순되던 결함, 회귀 테스트 3건). (3) 판정#58: 갭 랭커 최상위 BIAS 3건 동시 판정 —
  Kp 한 글자도 안 바꾸고 ①랭커의 used_for_calibration 필터 누락(순환) ②정의역 외삽
  ③Kp 상단치우침+고정연마입자 불일치로 각각 원인 특정, rank_only 처리. 수거 중 위임
  산출물의 결함 2건(실제 F4 접촉을 은폐 아닌 C4 신고로 처리, 죽은 노트 링크)을 직접
  적발·정정하고 데이터셋→노트 링크 실존 게이트를 신설. 실측: pytest 1069 passed
  (1032→+37, 회귀 0), completion **60/60**, qa_loop #257 --strict PASS ρ=0.9512 불변,
  격리 2→0개, 정확도 갭 24→21건(BIAS 3→0). 커밋 25806ae·b720ae8·f605738 전부 push.
...
## 2026-09-16 심야 04:00 [심야병렬] 서브에이전트 3명 동시 학습 — film-emerging Lv1-2 · film-poly-si Lv3-2 · slurry-abrasive Lv3-2
QA루프 #182 PASS(유의 7/25 ρ=0.9442 불변, 격리 2: us9200180b2 cu_abrasive/cu_h2o2 F4 — 판정#45로 null 종결된 영역, 3회차 연속 하락 없음). completion 49/50(C2 χ/cu_h2o2_bta 1칸 — 판정#43·#45 종결). corpus fetch **9/60**(직전 0/60에서 회복, ECS·JJAP·IOP 봉쇄 지속), extract 9(rates 대부분 0 — 소형 OA만), total 8960/with_fulltext 1519/queue fetch 7441·learn 1507.
대상: film-emerging(1/6 최저) + 5/6 잔여 중 Cal-1 외 Lv3-2가 남은 film-poly-si·slurry-abrasive(파라미터 근거화 성격). Max워커(03:34~ 판정#45 χ/cu)·성장엔진과 팩 무충돌(sim/·params yaml 수정 금지 지시). claude -p opus 3병렬(--max-turns 80), 13~22분.
- film-emerging Lv1-2: knowledge/materials/film-ru-mo-cmp-oxidizer-chemistry-ruo4-galvanic.md (출처8·코드6블록). Ru Pourbaix RuO₂ 1.12 V/RuO₄ 1.04 V; 산화제 8종 연마율(Cui 2013)·pH 8점(Cui 2012) — 정점은 최강 산화제가 아니라 E°≈1.6 V NaIO₄·NaClO(K₂S₂O₈은 정점의 3%); RuO₄ 억제창 = 알칼리 pH≳9(Peethala 2011, 이온화로 휘발성 상실). Mo: MoO₄²⁻ 용해 + MoO₃/Mo₂O₅ 부동태(W형, Ru·Co와 다름), RR/SER 41배(pH2·0.1M KIO₃). 갈바닉: Ru/Cu ΔE_corr 540→20 mV, i_g KIO₄ 8.48 > H₂O₂ 4.16 µA/cm²; Mo/Cu hold 116/polish 270 mV — 양극이 커플마다 다르고(Co/Cu→Co, Ru/Cu→Cu, Mo/Cu→Mo) 연마 부호도 반대(Co −72% vs Mo +133%). **Co/Ru 실측 ΔE_corr는 코퍼스 8960건+웹 전수 탐색 후 부재로 종결**(Lv1-1 빈칸). Mo/KIO₃ 농도–MRR은 판정#20 Langmuir(SSE 79) 실패, Hill n≈4.2(SSE 1.5) 채택 — 5점 적합이라 n 정확값 미검증. 신소재 팩 골격 후보표(Ru·Mo), 구현요청 P5~P7. PDF 6편 papers 등록.
- film-poly-si Lv3-2: knowledge/materials/film-poly-si-kp-preston-dissolution-rate-literature-reproduction.md (출처6·코드6블록). 알칼리 실리카 poly-Si (P,V,MRR) 역산 Kp 중앙 2.3e-13 m²/N(rpm→V r_cc 미확인이 최대 오차원 → estimated 유지). Preston a≈b≈1(Bae 2022) + Pirayesh 고속(≥100 rpm) 포화 → 저속·저압 유효창. 정적 식각률 0.115 nm/min(Jeon 2021) ≪ MRR(1/5000) → 정적용해가 아닌 시너지, Kp 기계형 채택. 아민/무기알칼리 종별차 EDA/NaOH 3.1배 레짐분리. 도핑 배율 undoped 1.0/p⁻ 0.60/p⁺ 0.20(Pirayesh 2014), n⁺는 2차인용이라 미선언. 팩 신설 제안 [P7] poly_si_silica.yaml(성장엔진 판정 대기).
- slurry-abrasive Lv3-2: knowledge/cmp/abrasive-parameters-to-kp-contribution-quantitative-model.md (출처14·코드6블록). 현재 κ 3항 재현: 농도항은 US9499721B2 E1에서 멱함수 SSE≈926 vs 포화형(Langmuir 255/포아송 135) ΔAIC>10 결정적 우세, 국소 지수 0.56→0.11 붕괴는 레짐 분할; 입경 정점 실리카 80/세리아 163 nm 레짐분할 유지; **입자 경도는 κ의 독립 승수가 아님**(코드 H^-1.5는 패드경도, Luo–Dornfeld H_w^-3/2는 웨이퍼경도 — 입자경도는 리지드 인덴터 성립 게이트). 3입자 배율표: 세리아 입자당 ≈23배(E4 오더표지), **알루미나 head-to-head 미확보로 빈칸**. D99→Δ 귀속(d50→Kp와 이중계상 금지). 구현요청 5~7.
품질게이트(check_knowledge --all 직접실행): **220/220 통과**. verify_claims 신규 3편 전원 PASS(DOI 28건 실존, 코드 18블록 0실패, 출처없는 수치주장 0). 429 흔적 없음.
체크박스 3개 [x], ORG §5 3행 갱신(film-emerging 2/6·film-poly-si 6/6·slurry-abrasive 6/6). sim/·knowledge/params 미변경(타크론 미커밋 변경 제외).
다음 회차 후보: film-emerging Lv2-1(GST·칼코게나이드) 우선. 5/6 잔여: film-nitride·slurry-chemistry는 Cal-1만(G2 대기) → defect-scientist·tool-endpoint·tool-post-clean Lv3-2. 성장엔진 인계: poly_si_silica.yaml 신설 판정[P7], κ 농도항 포화형 교체 제안, Ru/Mo 팩 골격.

## 2026-09-16 심야 01:00 [심야병렬] 서브에이전트 3명 동시 학습 — film-emerging Lv1-1 · slurry-colloid Lv3-2 · pad-lifecycle Lv3-2
QA루프 #174 PASS(유의 7/25 ρ=0.9442 불변, 격리 2: us9200180b2 cu_abrasive/cu_h2o2 F4 used_for_calibration 누락 — Max워커 판정#43 영역, 3회차 연속 하락 없음). completion 49/50(C2 χ/cu_h2o2_bta estimated 1칸 — 판정#43 승격 기각으로 종결 상태, 학습 주제 부적합). corpus fetch **0/60**(HTTPError 306·no fulltext 178 누적, attempts≥3 381건 / 잔여 7053) — 이번 회차는 OA 랜딩까지 전부 실패, extract 1, total 8960/with_fulltext 1526/queue learn 1514.
대상: 0/6 신규 film-emerging(G4 개방 09-15) 우선 + 5/6 잔여 중 Lv3-2가 파라미터 근거화 성격인 slurry-colloid·pad-lifecycle. 성장엔진(22:00 판정#42)·Max워커(00:42 판정#43 cu_h2o2_bta)와 무충돌. claude -p opus 3병렬(--max-turns 80), 20~24분(2명은 max-turns 도달했으나 산출물·체크박스·PROFILE 전부 완료 상태였음).
- film-emerging Lv1-1: knowledge/materials/film-co-interconnect-cmp-corrosion-galvanic-inhibitor.md (출처9·코드7블록, 572줄). Huang 2019 npj Mat.Deg. ab initio Pourbaix로 Co 부동태 창이 CMP pH 밖임을 정량, Gamagedara 2025/2026 Electrochem(MDPI OA) Cu/Co·Mo 갈바닉 — ΔE_corr는 필요조건이지 충분조건 아님(말론산 반례), 같은 슬러리에서 갈바닉 부호 역전(ΔE° +622mV vs 측정 −4~−33mV)은 레짐 분할로 판정. Cheng 2024 Appl.Surf.Sci. BTA 유도체 Langmuir K_ads·ΔG°_ads 재현(TTA만 3.00 kJ/mol 불일치, 오탈자 의심 표기), EIS 33.0배 vs PDP 12.1배 억제효율 2.7배 어긋남(Stern–Geary B 불변 가정 붕괴). 판정#17(평형 θ≠정상상태 MRR)이 Co에서 독립 재현(θ=0.917·SER 0.79·MRR 132.64 nm/min). **Co/Ru 실측 ΔE_corr는 코퍼스+웹 전수에서 부재** — 열역학 ΔE°=0.735V만, Lv1-2 최우선 과제로 PROFILE 기록. 구현요청 P1~P4(갈바닉 4입력 폐형식·hold/polish 플래그·Co χ팩 이중경로·PDP/EIS 출처 메타). PDF 5편 papers 등록.
- slurry-colloid Lv3-2: knowledge/slurry/slurry-storage-transport-history-effective-psd-model.md (출처5·코드9블록, 549줄). 이력→배율 함수형 요약표 H1~H9: 등가나이 t_eq=Σt·Q₁₀^((T−25)/10), Q₁₀=2.00(E_a 60.7 kJ/mol, US20250304827A1 E3); M_d50=(1+t_eq/τ)^(1/3) τ 95~110일(안정)/0.19일(불안정)(Granström 2015 ICPT 원문 확보); **꼬리(d99·LPC)는 d50 멱함수가 아닌 별도 상태량**(80°C×5일 LPC ×87 vs ×0.9); 세리아·고제타는 온도 가역(Seo 2021 E1, ±20°C·3주 ∓2.5~3% 복귀); 희석 문턱은 농도가 아니라 표면간거리 h=65nm; 전단 응집은 pH별 soft(가역)/hard(비가역) 플래그; DLS η(T) 미보정 시 +59% 가짜 성장. Othman 2024는 초록만(E5). 구현요청 (6). 미검증 4.
- pad-lifecycle Lv3-2: knowledge/materials/pad-usage-conditioning-history-time-dependent-kp-asperity.md (출처6·코드5블록, 539줄). 시간축을 **시간→패드상태(λ)→Kp 2단계**로 분리(Kp(t) 직접함수 기각). Sampurno 2011 λ(t) 원문 확보·R² 재현, Zhou 2018 무릎-압력, Wu 2013 디스크 2단계 마모, PHM 2016 477웨이퍼 교차검증. 판정#8로 제외된 pad_usage_hours/disk_usage_hours 드라이버의 재도입 경로를 구현요청으로 기록. 미검증 7.
품질게이트(check_knowledge --all 직접실행): **215/215 통과**. verify_claims 신규 3편 전원 PASS(DOI/특허 20건 실존, 코드 21블록 0실패, 출처없는 수치주장 0). 429 흔적 없음.
체크박스 3개 [x], ORG §5 3행 갱신(film-emerging 1/6·slurry-colloid 6/6·pad-lifecycle 6/6). sim/·knowledge/params 미변경(타크론 미커밋 변경은 제외).
⚠ 코퍼스 fetch 0/60 — 직전 회차 7~9/60에서 더 악화. HTTPError 306건 누적. 소형 OA 랜딩도 막힌 것인지 corpus.py의 _get UA/재시도 점검 필요(학습총괄 인계).
다음 회차 후보: film-emerging Lv1-2(Ru·Mo, Co/Ru ΔE_corr 실측 탐색 포함), 5/6 잔여 Lv3-2: pad-material·pad-structure·tool-platen-head·disk-kinematics는 Cal-1만 남아 G2 대기 → film-nitride·film-poly-si·slurry-abrasive·defect-scientist·tool-endpoint·tool-post-clean Lv3-2.
...
- 2026-09-16 [Max워커] **자기정정: 억제제 포화도 진단의 원인 귀속이 과잉이었다**
  (커밋 4eccbfd). 위 항목이 w_fe_oxidizer의 θ=0.9926을 "판정#17이 반증한 상태"로
  단정한 것은 틀렸다. 근거노트(w-cmp-picolinic-acid-...md §4)와 원문 Lee & Seo 2022
  (doi:10.3390/app12031227) §3.3을 다시 열어보니 **1.5 wt%→5.0 wt%에서 추가 억제 없음을
  직접 실측**한 논문이다 — 이 팩의 운전점이 문헌이 측정한 포화 플래토 위에 있고,
  민감도 0은 **모델이 맞는 것**이지 고장이 아니다. 진단을 고쳐 ①포화 신호(🔴)는 유지
  ②"(a) 실계 포화 / (b) K 오경로 — 이 진단은 둘을 구분하지 못한다"를 note에 명시
  ③문헌 귀속이 있는 팩(cu·w_fe)에만 원인 문장을 붙이고, 근거 없는 팩엔 단정하지 않는
  것을 테스트로 기계 고정했다. 같은 노트 §7의 별개 갭(모델이 0.5 wt%에서 이미 θ=0.978로
  조기포화하는데 원문 §3.2는 그 농도를 '억제 실패 수준'으로 서술 — 원문에 수치 없어
  확정 불가)은 유효하므로 "플래토 위에서는 맞고 문턱 아래에서는 미검증"을 note에
  항상 낸다. pytest 905 passed(904→+1), 격자 60/60 불변, qa_loop #211 PASS ρ=0.9442 불변.
  **교훈**: 모델 출력이 문헌과 같은 방향으로 둔감할 때 그것을 자동으로 '고장'으로
  읽으면 진단 자체가 거짓 경보원이 된다 — 신호와 원인 귀속을 분리해야 한다.

## 2026-09-16 14:xx [성장엔진] 판정#50 — sic_ceria_h2o2 χ 산화제(H2O2)축 부활 + 상속 결함 2건 동시 적발
판정#35가 "방향만 채택, 코드 미변경"으로 남겼던 죽은 축을 팩 **자신의** DOE 로 닫았다.
sic2026 DOE 50조건에서 H2O2 만 다르고 나머지 5축이 전부 같은 **18쌍**을 뽑아 촉진-포화형
Langmuir(자유 파라미터 K 하나, φ=0.15 고정)를 로그공간 적합 → K=0.7600. (2→4)/(4→6)/(2→6)
예측이 실측 중앙값과 각각 9.8%/4.5%/7.1% 차(전부 12% 이내), 로그 잔차 rms 31.3%,
부트스트랩 K 95% 구간 0.320~2.697 → confidence **estimated 고정**(캘리브레이션 데이터라
held-out 개선 근거로 쓰지 않는다). 판정#35의 곡률 반증은 실리카·무촉매 계라 계가 다르다 —
평균내지 않고 계로 쪼갰다.
부수 적발 2건: (a) oxidizer_wt_pct 본값이 3.0 wt%(출처가 구리 계 노트)인데 기준점은
4.0 vol% — 항을 켜는 순간 기준 배수가 1.0 이 아니게 되어 Kp 이중 계상이 될 상태였다.
(b) 자식 팩 sic_alumina_kmno4(KMnO4)가 H2O2 로 적합한 K 를 조용히 상속 — 판정#47과 같은
**산화제 종 게이트**를 sim/chemistry.py 에 신설해 차단(그 팩 형상은 갭으로 남긴다).
등급 하한 계산도 정정: _f_chi 가 켜지지도 않은 형상 키의 등급으로 χ 를 강등하고 있었다.
검증: H2O2 2/4/6 vol% → 1.6816/2.0228/2.1774 nm/min(축 부활), sic_alumina_kmno4 는 7.1323
완전 불변, pytest **908 passed**, qa_loop --strict PASS(유의 ρ=0.9442 불변),
완성 격자 **58→59/60**(C2 χ/sic_alumina_kmno4 해소).
부수: tests/test_inhibitor_saturation_diagnostic.py 의 "chemistry.py 파일 전체 동결" 테스트를
...
## 2026-09-18 [성장엔진] 판정#61 — sic_alumina_kmno4 pH 축 부활(C1 해소), Chen 2020 확보

판정#59 가 "알루미나계 pH 계수 미확보"로 정확히 지목한 갭을 이번 회차에 메웠다.
sci-hub 미러로 **Chen et al. 2020(Russ. J. Appl. Chem. 93(6) 832-837,
doi:10.1134/S1070427220060099)** 원문 확보 — 6H-SiC × 2 wt% Al2O3(100 nm) ×
0.05 M KMnO4 × pH 2/4/6/8/10, 조건당 3회 반복. pH 곡선은 인쇄 표가 아니라
막대그래프라 PDF 벡터에서 막대 상단·축 눈금을 뽑아 판독했고, 눈금 간격으로
독립 결정한 스케일이 본문 인쇄 최대값(Si면 1554 / C면 6412 nm/h)을 **-0.62 % /
-0.01 %** 로 재현했다.

- 형태: `g(pH)=φ+(1-φ)exp(-k(pH-2))`, `f=g(pH)/g(pH_ref)`. 순수 지수는 pH 10 을
  실측의 1/3 이하로 과소예측한다(산화력이 꺼져도 남는 **기계 압흔 경로**).
  Si면 5점 적합 → k=0.546/pH, φ=0.268(재현오차 -2.4~+2.6 %, rms(log)=0.0171).
- 신규: `sim/factors.py::_ph_sic_kmno4_acidic_term`(게이트=팩이 계수를 직접 선언할
  때만) + 팩 파라미터 3종(estimated) + `tests/test_sic_kmno4_ph_judgement61.py` 11건.
- **근거 충돌 판정**: 같은 팩 held-out Gong 2024 L25 는 pH 주효과가 평탄해 보이는데,
  ANOVA 로 재면 **pH·산화제·연마입자 세 인자 전부 비유의**(F=1.041/2.456/0.765,
  F crit 3.259). 즉 "효과 없음"이 아니라 "무반복 3인자 설계의 검정력 부족"이다.
  교란통제·스윙폭에서 앞서는 Chen 채택. 대가: gong2024 ρ -0.190 → **-0.264**(둘 다
  비유의) — 숨기지 않고 노트 §5 에 기록.
- 실측: χ status `unmodeled`→**partial**, 기준 pH 2.3 에서 χ=1.000 계약 유지,
  MRR pH 2/6/10 = 8.019/2.810/2.224 nm/min(**죽어 있던 축 부활**).
  completion **C1 0건**(같은 칸이 C2 estimated 로 이동, 격자 59/60 불변 —
  그래프 판독 근거라 등급을 올리지 않는 것이 정직하다).
  qa_loop #259 --strict **PASS**, 유의 held-out 평균 ρ **+0.957 불변**(9개/83조건).
- 판정#59 가 고정해 둔 테스트 2건을 **의도적으로 갱신**했다(무반응 고정 → 방향·기준
  1.0 고정). 남의 재료 곡선 차단 계약은 그대로 유지.
- 남은 갭: 이 팩의 **산화제(KMnO4) 형상축**은 여전히 죽어 있다(판정#50 종 게이트).
  Gong L25 3축 중 2축이 살아났고 1축이 남는다 — 다음 회차 표적.

### 2026-09-18 [Max워커] 판정#62 — sic_alumina_kmno4 입경축 세리아 상속 해소

- 판정#60 §5 가 남긴 마지막 정체성 결함. 입경 5키(`abrasive_size_nm`·`abrasive_ref_size_nm`·
  `abrasive_size_peak_nm`·`abrasive_size_exp_below_peak`·`abrasive_size_exp_above_peak`)가
  전부 부모(세리아) 소유였다 — 알루미나 팩인데 정점 163 nm·지수 +4/3/-1/3 이 세리아 형상.
- 근거: **Su Jianxiu et al. 2011**, Procedia Engineering 24, 441-446
  (doi:10.1016/j.proeng.2011.11.2673, gold OA) §3.2 Fig.2 — 6H-SiC(0001) + 알루미나,
  **입경 단독 스윕 4점**, 나머지 축 고정. 연마입자·막질이 정확히 일치(E3).
- 신값: 500.0 nm / 500.0 nm / 2500.0 nm / +0.3092888377 / -0.0664695242.
  5키를 **한 편집에서 함께** own 선언 — 값 하나만 바꾸면 알루미나 입경을 세리아 곡선에
  대입하는 꼴이 된다(판정#60 §5 금지).
- ⚠ **위임 산출물 결함 2건 적발**: ①1차 세션이 YAML 을 고쳤다고 보고했으나 한 줄도 안 바뀜
  ②Fig.2 판독값 W1.0=48 nm/h 가 그림 y축 눈금 최소치(50) 아래 = 불가능한 값. 재위임해
  PyMuPDF `get_drawings()` 마커 좌표 + 축 라벨 선형보정으로 **기계판독**하고, 보정식이
  원문 인쇄 2점(51.0·63.3)을 오차 0.09·0.38 nm/h 로 재현함을 verify 블록이 PDF 를 직접 열어
  assert 한다. **'판독했다'는 서술은 증거가 아니다 — 절차가 재현 가능해야 한다.**
- 회귀 고정: 감사 `_KNOWN` 에서 해소 5건을 빼되 **해소를 못 박는 신규 테스트**를 추가
  (`test_alumina_pack_size_axis_is_self_declared`). 목록에서 빼기만 하면 되돌려도 안 잡힌다.
- 게이트(Max워커 직접 실행): verify_claims ✓ check_knowledge ✓ **pytest 1094 passed 회귀 0**
  completion **59/60 불변** qa_loop #261 --strict **PASS ρ=0.9566 불변**. 커밋 8e0532e, push.
- 남은 갭: 이 팩의 **산화제(KMnO4) 형상축**(판정#50 종 게이트)과 C2 마지막 1칸
  χ/sic_alumina_kmno4(판정#59 §7-1, 산성 알루미나 pH 단독 스윕 문헌 필요).

### 2026-09-18 [Max워커] 판정#63 — 정체성 감사 축 사각지대 4키 판정

- 판정#60 §6 후속. `tools/audit_identity_inheritance.py` 의 축 미분류 **4건 → 0건**,
  신고 13건 → 19건.
- `ph_peak`·`ph_mrr_at_peak_rel` → **연마입자축**. 두 값이 Li et al. 2021
  (doi:10.1149/2162-8777/ac3e44) Fig.1 의 **20wt% 콜로이달 실리카** 곡선에서만 나왔고
  (정점 pH 11.0 = 1727 Å/min, 정점비 1727/1551 = 1.1135 vs 팩 1.113, 오차 0.045 %),
  판정#59 ③ 이 이 키의 산성 알루미나계 폴백이 **χ 산포 13.73배 오염**을 낸다를
  이미 실행으로 확인해 뒀다.
- `ph_softening_per_unit` → **막질축**(소유 팩 note 자신이 "SiC 표면 화학에서 나왔다"고 선언).
  ⚠ 이 배정이 **신고를 만들지 않는 것이 정확한 결과**다 — 상속 팩과 소유 조상의 `film` 이
  둘 다 sic_4h 로 실제 일치한다. 사각지대로 남아 우연히 조용한 것과 구분해 테스트로 고정.
- `pad_ra_m` → **AXIS_NONE**(소비처 전수 grep 결과 윤활 진단뿐, 5개 정체성 키 중 무엇도
  그 계산에 안 들어감).
- ⚠ 신규 신고 6건은 전부 현재 **비활성(dead)** — `_f_chi` has_own 우선(판정#34)이 각 팩의
  다른 own pH 분기를 먼저 고른다. **지금 조용한 것과 안전한 것은 다르다.**
- 부수: 기존 `AXIS_NONE` 4건 주석이 단정형이라 근거가 있는 것처럼 읽혔다 →
  `slurry_viscosity_pa_s` 만 근거 있는 축 없음으로 두고 나머지 3건은 **"미검증 — 귀속 근거
  미확보/상충"** 으로 명시. 특히 `damage_exponent` 는 팩 note 들이 **서로 다른 축**을 근거로
  같은 값을 전이해 왔다(강제 배정하면 그 상충 중 하나는 반드시 틀린다).
- 게이트(Max워커 직접 실행): verify_claims ✓ check_knowledge ✓ **pytest 1098 passed 회귀 0**
  completion **59/60 불변** qa_loop #262 --strict **PASS ρ=0.9566 불변**. 커밋 676bb2c, push.


## 2026-09-18 [Max워커] zeta(pH) 연속식 1차문헌 확보 — 엔진 등록은 **거부**(이온세기 조건 부재)

백로그 ㊀ 이행. `metal_contamination_surface.boltzmann_surface_enrichment()` 가 요구하는 zeta 가
4구간 정성 카테고리표(IEP 0 / 약산성 -20 / 중성 -40 / 약알칼리 -60 mV)로만 존재해 연속값
`slurry_ph` 를 매핑하려면 없는 임계값을 발명해야 하던 사각지대를 1차 문헌으로 메웠다.

**확보한 1차 자료 2건**(둘 다 OA, `papers/` + INDEX 등록):
- Gong, Wang, Liu, Song 2024, Materials 17(3) 679, doi:10.3390/ma17030679 (CC-BY) Fig.7b —
  **알루미나** zeta vs pH 실측 10점(pH 2.1~12.0). IEP 약 8.72(보간), 산성 기울기 -4.09 mV/pH
  (pH 2.1~8.0, n=6), 염기 기울기 -6.39 mV/pH (pH 9~12, n=4).
- Dawkins 2019 UAlberta PhD, doi:10.7939/r3-g3c2-xe63 Fig.4-2 — **세리아·실리카** zeta vs pH
  11점(pH 3~13). 세리아 IEP 본문 9.6 / 그림판독 9.81, pH7-10 전이 -13.9 mV/pH. 실리카는
  측정구간 전체에서 음전하(IEP<3), pH 5~12 평탄역 -58.0±1.7 mV.
  ※ 원 저널판 Dawkins et al. 2015 Appl. Surf. Sci. 345, 249 (doi:10.1016/j.apsusc.2015.03.170)
  은 Elsevier 유료·OA 부재라 미확보. 학위논문 4장이 그 논문을 그대로 재수록했다고 p.60 에
  **명시**해 등가 자료(E2)로 대체했고, 그 사실을 노트에 적었다.

**⚠ 등록하지 않았다 — 이유 3가지**(노트 §6):
1. **세 문헌 모두 이온세기(배경전해질 농도)를 보고하지 않는다.** pH 조정제 종류만 있고 mol/L
   이 없다. zeta 는 이온세기에 강하게 의존하므로 조건 없는 절대 mV 를 코드에 박으면 **다른
   조건에서 틀린 값**이 제품 출력이 된다. 부호·개형·IEP 만 신뢰 구간이 넓다.
2. 팩 스키마에도 이온세기 필드가 없다. `metal_contamination_surface.py` 상단이 이미 선언한
   스키마 부채("Recipe 에 pH·제타전위 시계열 필드가 없어 engine.Model 로 조립할 입력이 없다")가
   이번 조사로 해소되지 않는다.
3. 세 입자의 IEP 가 서로 달라(알루미나 8.7 / 세리아 9.6~9.8 / 실리카 <3) 단일 연속식으로
   합치면 입자종 분기가 필요해지고, 그 분기 키를 팩이 `has_own` 으로 선언하지 않는 한
   판정#59·#60·#62 에서 반복해 터진 **상속 오염**과 같은 경로가 된다.

따라서 1단계(문헌값 확보)로 종결. `sim/` 0줄 변경, YAML 0줄 변경, 격자 불변.
후속 우선순위(노트 §7): **이온세기 조건이 명시된** 세리아 1차 문헌 —
Suphantharida & Osseo-Asare 2004 JES 151 G658 (doi:10.1149/1.1785793), IOP 유료 미확보.

**⚠ 정직 기록 2건**(노트에 그대로 남김, 어느 쪽도 임의 채택 안 함):
- Dawkins p.64 본문이 "ceria zeta at pH 4 = 약 40 mV (Figure 4-2)" 라 서술하나 그 Figure 4-2 를
  기계 판독하면 **+52.1 mV** 다 — **30.2% 불일치, 원인 미상**. verify 블록이 이 불일치를
  `assert diff_pct > 20` 으로 **고정**한다(나중에 누가 판독을 바꿔 조용히 일치시키면 FAIL).
- 그림 판독은 육안이 아니라 `fitz.get_images()` + 테두리·눈금선 클러스터링 + 선형회귀
  보정식으로 **기계 판독**했고, 보정식이 **인쇄된 눈금값을 재현하는지**를 verify 블록이
  잔차 assert(알루미나 x 0.003 pH·y 0.023 mV, 세리아/실리카 y 0.09 mV)로 직접 검사한다.
  한계: 세리아/실리카 x 축은 눈금선이 없어 "본문이 pH 3~13 정수로 측정했다"는 텍스트
  진술에 의존한 배치임을 §4 에 명시.

게이트(Max워커 직접 실행): verify_claims OK · check_knowledge OK · **pytest 1098 passed / 1
skipped 회귀 0** · completion **59/60 불변** · qa_loop #263 --strict **PASS rho=0.9566 불변**.
⚠ 위임 세션은 노트 본문까지 쓰고 max-turns(120) 소진으로 중단(로그 30바이트) — Max워커가
출처태그 보강 2건·상호링크·게이트 5종을 직접 수행해 수거.


## 2026-09-18 [Max워커] K_i 경쟁 Langmuir 흡착상수 1·2회차 — Cr 1종 → 5종, scope 연도필터 거짓통과 3건 적발

백로그 ㉻ 이행. `sim/tier2_physics/competitive_metal_langmuir.py` 의 K_i 표가 **Cr 하나뿐**이라
"경쟁" 흡착이 성립하지 않던 상태를 1차 문헌으로 확장했다.

**확보 (5편, 전부 원문 PDF + INDEX 등록, Max워커가 fitz 로 1페이지 제목·저자 직접 대조):**
- Fe·Cu — Schindler et al. 1976, JCIS 55(2), doi:10.1016/0021-9797(76)90057-6 (실리카, 1 M NaClO4)
- Co — James & Healy 1972, JCIS 40(1), doi:10.1016/0021-9797(72)90172-5
- W — Gustafsson 2003, Chem. Geol. 200, doi:10.1016/S0009-2541(03)00161-X (KTH DiVA OA)
- **Al — Dugger et al. 1964, J. Phys. Chem. 68(4) 757, doi:10.1021/j100786a007** (실리카젤, BET 498 m2/g)
- **Mn — Coughlin & Stone 1995, ES&T 29(9) 2445, doi:10.1021/es00009a042** (침철석 TLM)

**핵심 성과는 값이 아니라 "섞으면 안 된다"를 확정한 것이다.** 다섯 문헌의 상수가 서로 다른
정의체계에 속한다 — A(Langmuir 사이트경쟁 L/mol) / B(Schindler 조건부 무차원, psi=0 근사) /
C(James&Healy 특이흡착 자유에너지 kcal/mol) / D(Gustafsson DLM 질량작용+정전보정) /
**D'(Coughlin TLM, 신설)** / **E(Dugger 열역학적 이온교환, m=n 일괄교환·C->0 외삽·이중층항 없음, 신설)**.
같은 열에 넣으면 물리적으로 무의미하므로 표를 6개로 분리했고, 환산 근거가 없어 **환산하지 않았다**.

**정직 표기 3건:**
- Co — James&Healy 가 SiO2 값을 스스로 "unreliable" 이라 명시 -> **K_i 미기재**(억지로 안 넣음)
- W·Mn — 흡착체가 페리하이드라이트·침철석(**철산화물**)이지 실리카·알루미나가 아님 = E4 타계 전이
- Cu 충돌 — Schindler -5.52 vs Sun 2007 -4.35 = **1.17 log(14.8배)** 차. 둘 다 E2 동급이라
  우열 판정 불가 -> **평균내지 않고** 원인(건식 Aerosil vs 습식 TEOS, 이온세기 1 M vs 0.01 M)을
  스코프 분리 기준으로 기록(CMP 슬러리=Sun, 고염 세정액=Schindler).

**Ru 는 1·2회차 모두 실패 — 다만 "없다"의 질이 올라갔다.** OpenAlex 전문검색 4종, 로컬 Ru CMP
1차문헌 4건 PyMuPDF 전수 스캔(zeta 언급은 실리카 입자 자체 제타전위이지 Ru 종 흡착상수 아님),
RuO4 기체상 트랩 문헌 2건 확인 -> **기체상이라 pH·이온세기가 구조적으로 존재하지 않아** 표
요건을 원리적으로 못 채움을 확인하고 폐기. `osti.gov` 전면 접속불가(HTTP 000). 3회차 경로
2개를 노트 §8 에 구체 기재.

**⚠ 감사 사각지대 적발 (이번 회차 최대 성과):** 위임이 §2 표에 적은 `scope --check ✓ 허용`
5건 중 **3건이 거짓 통과**였다. `tools/scope.py` 의 연도 필터는 **넘겨준 문자열에서 정규식으로
4자리 연도를 찾는다**(99~103행) — 제목만 넘기면 연도가 없어 **필터가 조용히 무력화되고 ✓** 가
나온다. 연도를 붙여 재실행하니 Dugger 1964 · Schindler 1976 · James&Healy 1972 가 전부
**✗ 제외(before_year 1990)**.
- **처리: 지우지도 승격하지도 않았다.** `agents/SCOPE.yaml` 무수정(사용자가 정한 경계를 임의로
  넓히지 않는다), 3건을 "범위 밖(pre-1990), 참고용"으로 §10 에 표기, 이 값들을 근거로 어떤
  팩터 confidence 도 올리지 않았다(`sim/` 에 소비처 0 — `C_i_free` 부재라 **오염 경로가 없다**).
- SCM 분야는 1960~70년대가 원 출처이고 2000년대 논문이 그걸 재인용하는 구조라
  `classic_exceptions` 추가를 **제안만** 기록(사용자 승인 사항).
- `tools/scope.py` 는 연도 미검출 시 ✓ 대신 "연도 미상 — 판정 불가"를 반환해야 한다고
  **신고만** 했다(판정#63 이 다룬 "축 미분류 = 조용한 통과"와 동형 구조, 다른 크론 소관).

**부수: DOI 오타 자체 적발.** Max워커가 INDEX 등록 시 Coughlin DOI 를 `es00009a041` 로 적었는데
Crossref 조회 결과 **무관한 논문**("Fate of Volatile Chlorinated Organic Compounds...")이 나와
`es00009a042` 로 정정했다(노트 본문은 처음부터 맞았다). 3건 전부 Crossref 실존 재확인.

**엔진 등록은 여전히 불가**(노트 §7): 금속이온 유리농도 `C_i_free` 필드가 팩 스키마에 없어
소비처가 0이다. `sim/` 0줄 변경.

게이트(Max워커 직접 실행): verify_claims OK(출처 10건 실존) · check_knowledge OK ·
...
### 2026-09-18 23:30~24:10 [Max워커] M3 게이트 2번째 — `sim/calibration/prior.py` (커밋 ec7ff90)

ORG.md §7.4가 요구한 3개 파일 중 두 번째. ingest.py(데이터 입구)에 이어 **물리 입구**를 세웠다.
ORG §7 서두가 "물리 prior가 있어야 소량 데이터로도 외삽이 된다 — 이게 Coventor·순수 AutoML 대비
차별점"이라고 적은 그 prior다.

**핵심 설계 판단: prior는 값을 바꾸는 물건이 아니라 그 값의 불확실성 폭을 선언하는 물건이다.**
- `mu` = 팩 YAML 선언값 **그대로**(rel=0 테스트로 고정), `source` 문자열도 소실 없이 보존.
- `sigma_log` = confidence 등급의 함수. **literature 등급만 근거가 있다** — EVIDENCE-RULES
  **판정#55**가 같은 재료계 문헌 3편(Gong 2024·Wang 2021·Chen 2015) 대조로 확정한
  "k*=MRR/(P·rpm) 문헌 간 정상 산포 ≈**1.5배**"를 1σ 로그폭으로 그대로 승격했다(ln1.5=0.4055).
  나머지 등급의 상대배율(verified 0.25 / measured 0.5 / estimated 2.0)은 **순단조만 주장**하고
  절대 폭은 "미검증 — 이 프로젝트의 운영 규약이지 문헌값이 아니다"를 코드 상수 주석·docstring·
  `rationale` 문자열 3곳에 명시했다.
- `confidence=unverified`는 **prior를 만들지 않는다**(`PriorExcluded`). 근거 없는 값에 그럴듯한
  분포를 씌우는 것이 정확히 COMPLETION.md가 말한 오염이다.
- `prior_predictive_mrr`은 **Kp 하나만** 엔진에 통과시킨다. Kp는 Preston MRR=Kp·P·V의 순수
  축척 상수라 `series_scale.py`의 "구조는 물리, 축척은 데이터" 경계와 정확히 들어맞는다.
  나머지 85~90개 파라미터는 prior는 만들되 **엔진에 통과시키지 않고** `excluded_params`에
  사유를 남긴다 — 지수·게이트·재료축을 일괄 로그정규로 흔들면 `series_scale.py`가 금지한
  "계열별 물리 상수 부여"를 이 함수가 프리셋 형태로 우회하게 되기 때문이다.

**실측(직접 실행)**: `oxide_silica`(Kp **verified**) 결정론 143.013 vs prior 중앙값 143.278 ·
CI90 125.1~158.6 (±12%). `cu_h2o2_bta`(Kp **estimated**) 500.545 vs 508.039 · CI90 172~1147 (≈3배).
**중심은 안 옮기고 폭만 등급을 따른다** — prior가 모델을 이동시키지 않음을 수치로 확인.

**게이트(Max워커 직접 실행)**: `tests/test_prior.py` **40 passed**, 전체 **1153 passed / 1 skipped**
(1113→+40, **신규 회귀 0**), `completion.py check` **59/60 불변**.
⚠ 기존 3건 실패(`test_sic_kmno4_ph_judgement61`)는 앞 항목과 동일하게 다른 크론의 미커밋
워킹트리에서 온 것이며 이 커밋 경로와 무관하다.

**위임 품질 개선 확인**: 브리프에서 "전체 스위트를 돌리지 말고 신규 테스트만 돌려라"로 바꾸자
위임이 **4회차 만에 정상 종료**했다(로그 1744바이트, 요약까지 작성). 직전 4회 조기종료의 원인이
모델의 한계가 아니라 **9분짜리 전체 스위트 대기**였음이 확인됐다 — 이후 위임 브리프는 전부
"신규 테스트만, 전체 회귀는 호출자가" 규칙을 넣을 것.
...
### 2026-09-19 09시 회차 [Max워커] — PTW 패턴밀도를 MRR 경로에 연결 + 판정#65

커밋 c111179 (push 완료). **M3 사슬의 마지막 실질 공백을 메웠다.**

직전 회차(커밋 81c3afd)가 "PTW와 NPW의 `removed_nm`이 비트 단위 동일"을 실측 기록했다 —
패턴밀도가 진단 필드로만 나오고 MRR 경로에는 연결돼 있지 않았다는 뜻이고, 그 탓에
`sim/calibration/fit_ptw.py`의 PTW 층은 **인위 주입 코사인으로만** 검증된 상태였다.

**한 일**
1. 신규 opt-in Model `tier1.pattern_density_effective_pressure` 등록.
   Preston MRR=Kp·P·V에서 P→P_eff 치환 = up-area 제거율 = ratio × blanket MRR.
   Kp가 blanket NPW에서 역산된 값이라 이중계상이 아니다.
   근거: Sorooshian(2005) PhD dissertation §3.3 표 3.3~3.6 실측.
2. 게이트는 **ValueError로 크게 실패**한다 — NPW / density 미지정 / 표밖 density /
   표밖 압력 4경로 전부 실행 확인. 조용한 blanket 폴백은 "패턴 효과를 넣었다"는
   거짓말이 되기 때문이다. 온도는 표 상온행 23℃ 고정이고 **ΔT_ss를 끌어다 쓰지 않았다**
   (표 온도 10/23/35/45에 실제 패드 표면온도를 매핑할 근거가 없다).
3. **판정#65** — 이로써 패턴 모델이 둘이 되어 같은 양에 최대 2.50배 불일치가 생겼다.
   기록만 하지 않고 EVIDENCE-RULES 서열로 판정: Boning 1/ρ(기하 가정 폐형식, **E4**)
   < Sorooshian 실측표(대상계 실측, **E3**). 실측 대비 1/ρ 과대배수는
   ρ=0.10 **2.50배** / 0.50 1.29배 / 0.90 0.95배 — **저밀도일수록 어긋나고**, 이는
   1/ρ가 ρ→0에서 발산하는 구조 결함(모듈의 RHO_MIN=0.15 클램프가 가리던 바로 그 문제)이다.
   **1/ρ를 삭제하지 않고 경고만 달았다** — 실측표는 3점 격자·2압력만 다루므로 연속 ρ
   폴백이 필요하다(§3 스코프 분할). 두 값을 평균내지 않았다.
4. 부수 버그: `simulate()`의 "PTW인데 패턴 모델을 안 썼다" 경고가 문자열 비교라
   **신규 모델을 골라도 거짓 경고가 계속 붙었다** → `_PATTERN_MODELS` 집합으로 교체.

**불변식 유지**: 기본 모델 `tier1.preston_radial` 출력 비트 불변,
`sim/tier2_physics/npw_ptw_effective_pressure.py` 0바이트 수정, 팩 YAML 새 키 0건.

**게이트(Max워커 직접 재실행)**: pytest **1263 passed / 1 skipped, 회귀 0** ·
completion **64/70 불변**(순수 모델 추가라 confidence 무변경 — 격자가 움직였으면 버그) ·
qa_loop #271 --strict **PASS ρ=0.9512 불변**.

**후속**: `fit_ptw.py`는 여전히 기본 모델을 쓴다 — 캘리브레이션 사슬을 이 모델로
전환하는 것은 별건이다. 그리고 실측표가 다루는 밀도는 3점뿐이라, 연속 밀도가 필요한
...
사슬(`sim/calibration/fit_ptw.py`)에 실제로 넘겨본 결과에서 나왔다.

**증상**: 밀도 격자 **안** 값(0.50)인데도 ValueError.
**진짜 원인**: 격자가 아니라 타입이었다. `Recipe.meta` 는 `Dict[str, str]` 로
**선언돼 있고**(engine.py:83) 생산자 3곳(`sim/cli.py`·`sim/studio.py`·`fit_ptw.py`)이
전부 `str(...)` 로 넣는데, 소비자 2곳이 캐스팅 없이 `(0.10, 0.50, 0.90)` 리터럴과
비교했다 — `'0.5' != 0.5`. 같은 파일의 `_cu_dishing_erosion_tugbawa_diagnostic` 은
이미 `float(meta[...])` 로 읽고 있어 **한 파일 안에서 규약이 갈려 있었다.**

⚠ **더 나쁜 증상이 사용자 경로에 있었다.** CLI·Studio 에서는 예외조차 나지 않고
유효압력 진단이 **조용히 스킵**되면서, 붙는 사유가
`"pattern_density=0.5는 표에 없는 값(지원: 0.10/0.50/0.90)"` —
**지원 목록에 있는 값을 없다고 말하는 거짓 안내**였다. 사용자가 슬라이더를 정확히
0.5 에 놓아도 진단이 안 나오고, 그 이유는 틀리게 설명되는 상태였다.

**조치**: `sim/engine.py` 에 `_meta_pattern_density()` 헬퍼를 두고 진단 함수와 신규
모델이 둘 다 그것을 쓰게 했다. **생산자는 고치지 않았다** — 생산자는 선언된 타입
규약을 지키고 있었고, 3곳을 고치면 규약과 코드가 더 어긋난다. 숫자로 안 읽히면
지어내지 않고 None + 스킵 사유.
실측: `'0.5'`·`0.50` 둘 다 ratio=1.589047619047619 일치, `'0.10'`→2.4214…,
`'abc'`·`'0.3'`→None.

⚠ **연결됐다고 정확도가 좋아진 것은 아니다.** 이 모델은 전 반경 **상수배**
(`np.ptp(ratio) < 1e-9`)라 반경 잔차를 학습하는 GP 보정에 새 정보를 주지 않는다.
그래서 `fit_ptw` 의 `model` 기본값을 바꾸지 않았고, 그 근거를 모듈 docstring 에 적었다.

**게이트(Max워커 직접 재실행)**: pytest **1267 passed / 1 skipped, 회귀 0** ·
completion **64/70 불변** · qa_loop #272 --strict **PASS ρ=0.9512 불변**.

**위임 운용 관측**: 위임은 증상을 정확히 측정하고 정직하게 보고했으나
"지시 범위 밖"으로 판단해 **문서화만 하고 종료**했다. 측정은 맞았고 판정이 빠졌다.
다음 브리프에는 "원인이 설계된 제약인지 결함인지까지 판정하라"를 명시할 것 —
**둘은 증상이 같다(둘 다 ValueError).**


### 2026-09-19 12시 회차 [Max워커] — 판정#68: κ 입자 형상 축 판정 (미배선 종결)

커밋 dfb4733 (push 완료). 코드·YAML **0줄 변경**이 결론이다.

**갭**: `sim/factors.py::_f_kappa` 입경 항 주석이 스스로 "적용 범위 밖(응집체 비율 높은
슬러리)에는 **형상 팩터가 따로 필요하다**"고 적어 두고 비워 둔 칸. 기존 노트
(abrasive-shape-effect-purespherical-subset-nonmonotonic.md §2)는 형상 효과의 **방향이
2차 인용끼리 정면 모순**(RSC 리뷰 59% 감소 vs ScienceDirect 초록 증가)이라 판정#2를
보류로 남겨 뒀고, 그 상태가 3회차째였다.

**Max워커 신규 실측이 출발점**: 기존 노트는 비구형(누에고치형·응집체) 배합을 **제외만 하고
한 번도 비교하지 않았다**. `tw202115224a_cu_abrasive_size_pressure.yaml`(n=18, 조성·rpm·
유량 전부 고정, 변하는 건 연마입자와 압력뿐)을 형상 축으로 재분류하니 —
2.5 psi 구형 577.3 vs 비구형 818.8(**1.418배**), 1.5 psi 374.2 vs 513.1(**1.371배**),
압력별 정규화 후 Mann-Whitney 단측 **p=0.000377**(배수 1.395). 두 압력에서 독립적으로
같은 방향·비슷한 배수가 나온다. 같은 정규화에서 구형 내부 입경-MRR은 ρ=+0.087, p=0.787로
**여전히 null**(판정#1 유지).

**판정**: E3(대상계 실측) > E5(2차 인용) 1단계 즉결 — "비구형이 구형보다 MRR이 높다".
RSC 리뷰의 59% 감소는 폐기. 위임이 타계 독립 1차 문헌 1편(doi:10.3390/ma15217525, 세리아/
유리, gold OA 로컬 전문)에서 같은 방향 본문 실측 서술을 찾아 보강했다. 부수로 같은 입경
50 nm 짝(Nalco 구형 441.4 vs Fuso 누에고치형 845.3 = **1.915배**, 입경 교란 없는 유일한 쌍)도
확인 — 형상 효과가 입경 교란의 착시가 아니라는 방향.

**배선하지 않았다 — 이것이 이번 회차의 핵심 판단이다.** 정밀 배수 1.395의 유일한 출처가
`used_for_calibration: false`인 **held-out 데이터 자신**이라, 팩에 박으면 그 데이터로 재던
ρ가 오르는 것이 자기 답안지 채점이 된다. 독립 문헌(ma15217525)은 방향만 주고 배수를 안 준다
(Figure 7b 그래프 전용). 현재 어느 팩도 `abrasive_shape`를 own 선언하지 않으므로 전 팩
스킵이 정상이고 MRR은 비트 불변. Kim et al. 2021(doi:10.1016/j.powtec.2020.11.058) 원문은
이번 회차도 5경로 전부 미확보(OpenAlex/Unpaywall/S2 전부 is_oa:false, sci-hub 3미러 캡차,
저자기관 pure.skku.edu 403).

**교훈: "판정"과 "배선 가능"은 별개다.** 방향을 확정해도 배선 가능한 **독립** 배수가 없으면
미배선이 정직한 결론이다. 이 경우 held-out 오염 방지가 "ρ 올리려 약한 근거 채택 금지"와
같은 자리를 차지한다.

**부수 관측**: `sim/web/studio3d.html:254`에 `abrasive_shape` UI 드롭다운이 이미 있는데
`pk:null`로 엔진 미연결이고 "no calibrated exponent"라고 스스로 적어 둔 **죽은 필드**다.
이 노트의 결론과 우연히 일치한다 — 즉 UI가 먼저 정직했다.

**게이트(Max워커 직접 재실행)**: verify_claims ✓(출처 2건 실존·코드 2블록 통과) ·
check_knowledge ✓ · pytest **1267 passed / 1 skipped, 회귀 0** · completion **64/70 불변** ·
qa_loop #276 --strict **PASS ρ=0.9566 불변**(배선 안 했으므로 불변이 정상).

**위임 운용 관측**: max-turns 120 소진으로 죽었으나(`Error: Reached max turns (120)`),
브리프의 "5턴 안에 노트 생성 + 확보할 때마다 즉시 저장" 지시 덕에 **노트 234행·판정 행이
전부 남았다**. 사망했는데 산출물은 온전한 첫 사례다 — 그 지시를 계속 쓸 것. 게이트 6종은
Max워커가 직접 실행해 수거했다.


### 2026-09-19 12시 회차 (2) [Max워커] — 판정#69: χ/sic_alumina_kmno4 pH 승격 2회차 실패

커밋 00ce432 (push 완료). **값·confidence 0 변경 — 정직한 미전진이다.**

**표적**: `completion.py check` 가 신고하는 C2 4칸 중 `χ chi/sic_alumina_kmno4:
confidence=estimated`. 이 칸이 estimated 인 이유는 모델이 없어서가 아니라(판정#61 이
`_ph_sic_kmno4_acidic_term` 으로 이미 배선했다) **수치 출처가 Chen 2020 Fig.1 막대그래프
판독**이라서다.

**결과**: 승격 실패. Chen 2020(doi:10.1134/S1070427220060099) 원문을 다시 열었으나 본문
인쇄값은 pH 2 최댓값 2개(Si 1554 · C 6412 nm/h)뿐이고 **중간 4점은 그래프 전용**임을
재확인했다. 가장 유망한 대안으로 Wei et al. 2026(Int. J. Adv. Manuf. Technol.,
doi:10.1007/s00170-026-17750-1 — 4H-SiC Si면 + Al2O3, pH 전용 연구)을 찾았으나
Unpaywall·OpenAlex·Semantic Scholar·CORE·sci-hub 2미러·ResearchGate 전부 봉쇄.

**종결하지 않았다 — 2회차다.** 위임이 `sim/factors.py` 코드 추적으로 이 칸의 실제 게이트
표적이 `sic_kmno4_ph_acid_k` 임을 확인하고, 판정#61 = 1회차 / 이번 = 2회차로 셌다.
3회차 미도달이므로 `validation/C2-CLOSURES.yaml` 종결은 하지 않고, 노트 §4에 **다음 회차의
구체 경로 3개**를 남겼다(Wei 2026 그린 OA 재시도 시점·Springer 초록 접근 정책 변동·
ceramint 2025.07.097 재시도 — 단 직교표라 판정#61 급 ANOVA 비유의성 점검 선행).
"다음에 또 찾아보기" 류의 무내용 보류가 아니다.

**부수 발견**: 위임이 판정#64(φ 농도 보간)가 **코드·YAML 에는 실존하는데 원장에 누락**된
감사 결함을 발견했다. 과제 범위 밖이라 고치지 않고 노트에 기록만 남겼다 — 다음 회차 후보.

**게이트(Max워커 직접 재실행)**: verify_claims ✓(출처 6건 실존·코드 1블록 통과) ·
check_knowledge ✓ · pytest **1267 passed / 1 skipped, 회귀 0** · completion **64/70 불변**
(승격 실패이므로 65 로 오르지 않는 것이 예상된 결과) · qa_loop #277 --strict **PASS
ρ=0.9566 불변**.
...
### [Max워커] 판정#72 — C4 `cu_h2o2_bta` held-out ρ 0.7175 원인 진단 (2026-09-19)

C4 미충족 원인을 5개 데이터셋 전수 분해로 특정: 유의(p<0.05)한 held-out은
`tw202115224a_cu_abrasive_size_pressure`(n=18) 하나뿐이고, 완성격자가 보고한 0.7175는
바로 그 데이터셋의 ρ 그 자체다(평균이 아니다, n_sig=1). 형상(구형 vs 누에고치형/응집체)
축으로 재분류·재검증(Mann-Whitney p=0.000377, 배수 1.395배, 50nm 동일입경 직접대조
1.915배)한 결과가 **판정#68이 같은 데이터셋으로 이미 확정해 둔 입자 형상(κ) 축 결론과
수치까지 일치** — 신규 결함이 아니라 기종결 사안의 재발이다. 판정#68의 결론(진짜 물리
효과이나 정밀 배수의 유일한 출처가 이 held-out 자신이라 배선하면 자기채점이 되어
배선 안 함)을 그대로 유지, **(c) 구조적 한계로 판정. 코드·YAML 미변경.**

별도로 비유의 데이터셋 `jani2025_cu_rsm_composition_heldout`(ρ=−0.3494, n=13, p=0.877,
C4 평균에 미반영)에서 독립적으로 발견한 (a)형 결함 — ψ에 글리신(착화제) 억제 항이
빠져 있던 것 — 은 이번 회차에 `sim/chemistry.py::_chelator_suppression_term`으로
배선했다(Jani 2025 통제쌍 2건 + 회귀계수 [glycine]=−440.91, p=4.08e-7, 계수 역산에
쓴 4조건은 held-out에서 제외해 자기채점 방지). 물리적으로 정당한 수정이지만 이
데이터셋이 애초 비유의라 **C4 숫자에는 영향 없음** — (a) 수정과 C4 결과를 섞지 않고
분리 보고했다.

**결과**: completion **66/70 불변**(C4 그대로 실패 — 숫자로 실패를 숨기지 않는다).
신규 회귀 `tests/test_cu_glycine_chelator_suppression.py` 4건 PASS(기준배수 항등원,
착화제 종 게이트, 농도-억제 단조성, 통제쌍 5% 재현). qa_loop --strict PASS ρ 불변
(글리신 항은 이 두 held-out 조건에서 예측 벡터를 바꾸지 않음). pytest 전체 결과는
아래 게이트 실행 기록 참조.
노트: knowledge/cmp/c4-cu-h2o2-bta-heldout-rho-diagnosis.md

### 2026-09-19 (성장엔진, 판정#72) — ψ 네 번째 갈래: 착화제(글리신) 억제축 배선

C4 진단(판정#72)이 부수적으로 찾아낸 (a)형 결함을 실제로 고쳤다. `cu_h2o2_bta` 팩은
기준 조성에 글리신 0.1332 M 을 선언하고 있었지만 그 **농도가 MRR 에 전혀 연결되지
않았다** — `chelator_M` 은 산화제 항의 레짐 게이트 판별용으로만 읽혔다.

**방향이 직관과 반대다.** Jani 2025(doi:10.1149/2162-8777/adc59e) Table I×II 에서
연마입자가 있고 글리신만 바뀌는 통제쌍은 정확히 2건(35실험 전수탐색)이고, 둘 다
**억제**다: 0.13 M → 0.8605배, 0.26 M → 0.6513배. 같은 논문 회귀 `[glycine]` =
−440.91 (p=4.08e-7) 과 원문 결론("glycine effectively functions as an inhibitor
rather than a dissolution promoter")이 독립 확증한다. 회귀계수 6개를 직접 재적합해
전부 1% 이내로 재현했다(노트 §5 블록2) — 인쇄값 전사가 아니다.

**함수형은 지수 1개**(`exp(-a·(C-C_ref))`, a=1.5119 /M). Langmuir/Hill 피복형을 쓰지
않은 이유는 2점으로는 (K,k)가 분리되지 않기 때문이다 — K를 0.13→32 /M (240배) 움직여도
SSE가 7.7배 안에 다 들어온다. 모수를 데이터 점보다 적게 유지했고, 잔차는 −4.5%/+3.6%.
착화제 **종 게이트**를 걸었다: 옥살산은 같은 회귀에서 +536.63 으로 부호가 반대라
(판정#45) 전이하지 않는다.

**held-out 은 실패했고 숨기지 않는다.** 같은 논문 RSM 에서 적합에 쓰지 않은 13조건을
`jani2025_cu_rsm_composition_heldout.yaml` 로 등록 → ρ = **−0.349** (p=0.877, n=13,
비유의). 원인은 글리신 항이 아니라 **옥살산 미모델링**으로 판단한다: 이 13조건에서
옥살산이 4배 변하고 회귀계수가 전 인자 중 최대(+536.63, p=1.7e-7)인데 대응 항이 없다.
글리신 항을 넣기 전에도 같은 조건의 ρ는 음수였다(−0.15) — 이 항이 순위를 망친 게 아니다.
**다음 회차 C1 최우선 후보 = 옥살산(착화제 촉진축).**

또 `tests/test_factors.py::test_psi_metal_packs_unchanged_by_oxide_extension` 의
계약을 좁혔다 — "term 이 inhibitor 하나여야 한다"는 원래 의도(산화막 경로 배제)의
대리 조건이었으므로, `adsorption_shield` 부재를 직접 금지하도록 바꿨다.

**게이트 실측**: pytest 전체 **1284 passed / 1 skipped** (594s, 신규 7건 포함),
`verify_claims` ✓(코드 4블록·출처 1건 실존), `check_knowledge` ✓,
`qa_loop --strict` **PASS** 유의 평균 ρ **0.9566 → 0.9566**(불변 — 신규 데이터셋이
비유의라 집계 밖, 기준 조건 배수는 항등 1.0), completion **66/70 불변**(C4 여전히 실패).
노트: knowledge/cmp/psi-glycine-chelator-suppression-cu-jani2025.md

---

### [Max워커] 번호 정정 — 위 "판정#72 — C4 진단" 항목은 **판정#73**이다 (2026-09-19)

위에 있는 "[Max워커] 판정#72 — C4 `cu_h2o2_bta` held-out ρ 0.7175 원인 진단" 항목은
작성 당시(직전 커밋이 #71) 스스로를 #72로 표기했으나, 같은 시각 이 저장소에서 별도
작업자(성장엔진)가 독립적으로 진행하던 글리신 억제축 작업이 먼저 커밋되며 #72를
가져갔다(커밋 cab964e·b838c89, 바로 아래 "성장엔진, 판정#72" 항목). 두 작업은 완전히
다른 발견(C4 원인 진단 vs ψ 착화제축 신설)이라 번호를 공유하면 안 되므로, **C4 진단
항목의 올바른 번호는 판정#73**이다. `EVIDENCE-RULES.md`에는 처음부터 #73으로
등록했다 — 위 MASTER-PLAN 항목의 "#72" 표기만 커밋 경합으로 잘못 남았고, 내용
(원인 특정·판정#68 재현·(c) 판정·격자 불변)은 그대로 유효하다.

게이트 재확인(이 정정 시점, 두 워커의 변경이 모두 반영된 상태로 전체 재실행):
pytest 전체 **1284 passed / 1 skipped**(회귀 0 — 첫 전체 실행에서 동시 실행 중이던
다른 pytest 프로세스와의 파일 경합으로 `test_psi_metal_packs_unchanged_by_oxide_extension
[cu_h2o2_bta]`가 1회 일시적으로 실패했으나, 격리 재실행·전체 재실행 모두 통과해
...
### 2026-09-19 (3회차, 성장엔진 · 판정#77) — C4 새 held-out 확보 시도 → **pH 축 침묵 발견**

C4(`cu_h2o2_bta` 유의 평균 ρ 0.7175 < 0.85)를 새 held-out 으로 돌파하려 했다.
신규 데이터셋 `validation/datasets/miranda2004_cu_ph_h2o2_2x2.yaml`
(Miranda/Imonigie/Moll 2004 IEEE WMED, doi:10.1109/WMED.2004.1297359, OA 전문 확보 →
`papers/miranda2004-wmed-cu-cmp-ph-h2o2.txt`). 2^2 완전요인 pH 4/8 × H2O2 1.5/3.5 %,
3반복 12 run, 장비·패드·슬러리 베이스 전부 고정.

**이번 회차의 실질 성과는 ρ 가 아니라 진단이다.** 백테스트에서 pH 4 와 pH 8 의 예측이
소수점까지 동일하게 나왔다(1069.6035869782888 / 662.1884867892296 이 각각 두 번).
원인은 버그가 아니라 `sim/factors.py:1152~1176` 이 스스로 선언한 게이트다 — 이 팩은
`inhibitor_mM=1.0`(억제제 유)이라 pH 8 이 **레짐 4(알칼리×억제제 유, 관측 없음)** 로
떨어져 pH 항이 꺼지고, pH 4 는 레짐 1 이지만 `ph_ref=4.0` 이라 배수가 항등적 1.0 이다.
결국 네 조건 전부 pH 배수 1.0 → ρ=0.000, τ=0.000, p=0.5417, MAPE 928.5 %, 계통편향 0.17 배.
절대값은 pH 8·3.5 % 에서 실측 24.3 대비 예측 662.2 nm/min 으로 **27.2 배 과대**.

문헌 관측 자체는 강하다: 같은 재료계·장비에서 H2O2 를 1.5→3.5 % 로 올릴 때
**부호가 pH 로 뒤집힌다** — pH 4 에서 ×1.489(촉진), pH 8 에서 ×0.1394(억제),
로그공간 교호작용 대비 2.368 = 민감도 10.7 배 차. 원문 ANOVA 도 pH×H2O2 p=0.0207 로
교호작용을 독립 확인한다. 즉 **Cu 산화제 항의 부호는 착화제 종만으로 정해지지 않고
pH 도 스위치다** — 판정#41·#45·#47 이 착화제로 가른 것과 같은 현상의 pH 축 버전이고,
현행 `_oxidizer_term` 의 게이트(`slurry_ph<6 AND chelator_M>0`)는 이를 담지 못한다.

**계수는 만들지 않았다(E4 — 채택 금지).** Cabot 5001 이 상용 비공개라 억제제 유무를
확정할 수 없어 이 데이터가 레짐 4 인지 레짐 2 인지 모르고, pH 2 점 1 미지수는 적합이
아니라 항등이며, 교호작용이 있는데 주효과만 뽑으면 계수가 데이터셋 지문이 된다.
판정#68 과 동형의 보류다.

게이트: verify_claims ✓(assert 14 항) check_knowledge ✓ qa_loop #301 `--strict` **PASS**
유의 평균 ρ **0.9566 불변**(이 데이터셋 비유의라 집계 밖) completion **67/70 불변**
(C4 그대로 실패 — 숫자로 숨기지 않는다) 감사 clean.
노트: knowledge/cmp/chi-cu-ph-oxidizer-interaction-miranda2004.md


## 2026-09-19 [Max워커] 판정#64·#72 원장 누락 복구 + 원장-참조 정합 테스트 · 판정#76 κ 형상축 2회차

**① 추적 결함 복구 (커밋 4b1821d, push)**

`sim/factors.py:1280`·`knowledge/params/sic_alumina_kmno4.yaml`·노트 2편·테스트 2편·
MASTER-PLAN 여섯 곳이 근거로 인용하는 **판정#64**(φ 산화제 농도 보간, 커밋 86291ce
2026-09-18)가 EVIDENCE-RULES.md 판정표에 **행이 없었다**. 판정#69-종결이 부수 발견으로
지목했으나 범위 밖이라 고치지 않은 건이다.

행 내용은 창작이 아니라 `git show 86291ce` 커밋 전문과 근거 노트
(`sic-kmno4-ph-floor-oxidizer-coupling-wang2021.md`)에서 직접 대조해 기술했다.

**같은 결함이 하나 더 있었다** — 신설 테스트를 처음 돌리자 **판정#72**(글리신 착화제
억제항, 커밋 cab964e·b838c89)도 함께 잡혔다. 판정#73 행이 이미 "판정#72"를 근거로
인용하는데 #72 행 자체는 등록된 적이 없었다. 두 행 모두 **사후 복원임을 행 안에 명시**
했다 — 원장이 스스로 지연을 숨기면 복구의 의미가 없다.

`tests/test_evidence_ledger_integrity.py` 신설. 저장소 전체(`sim/`·`knowledge/`·`tests/`·
`tools/`·`validation/`·최상위 md)의 `판정#N` 참조를 수집해 원장 행과 대조하고, 없는
번호를 참조하면 **파일:행과 함께 FAIL**. 표기 두 갈래를 구분 처리한다:
- **접미사형**(`#69-종결`·`#49-B`) → 기반 번호가 원장에 있으면 통과
- **분할행형** → 글자가 직접 붙으면(`24A`) 정확한 행을 요구하고, 순수 숫자(`#24`)는
  기존 총칭 관례로 인정. 그래야 `24D` 같은 실존하지 않는 분할행을 여전히 잡는다.

**복원 전 실제로 FAIL 하는 것을 확인한 뒤 복원했다** — FAIL 을 못 본 테스트는 아무것도
지키지 않는다.

**② 판정#76 — κ 입자 형상 축 2회차 (커밋 d5df343, push)**

C4 마지막 칸(`cu_h2o2_bta` 유의 평균 ρ=0.7175<0.85)의 유일 원인은 판정#73이 특정한
형상 축 미모델링이고, 판정#68이 방향은 확정했으나 정밀 배수의 유일 출처가 held-out
자신(TW202115224A)이라 자기채점 회피로 배선을 보류한 상태였다. **판정#75 선례**
("자기채점이라 배선 못 한다"는 "다른 문헌을 찾아라"는 뜻)를 형상 축에 적용했다.

판정#68이 "그래프 전용이라 못 읽는다"고 판단한 ma15217525(Materials 2022) Figure 7b 를
MDPI CDN 원문 PDF 신규 확보 후 **축 눈금 라벨 선형회귀 + 색상 마스크 이진부식**으로
판독해 **배수를 처음 확보**했다. 판독 정확도는 본문이 인쇄한 유일한 숫자(555 nm/min)를
553.6 으로 재현(오차 0.25 %)해 교차검증했다.

**그런데 그 배수가 상수가 아니었다** — 농도 1~9 wt% 에서 2.18/2.50/1.52/1.33/1.09 배로
비단조이고, TW202115224A 운전점(0.0833 wt%)은 그 스윕범위 **밖**이라 대응값을 특정할 수
없다. 여기에 대상계 불일치(세리아/유리=기계마모 지배 vs Cu/실리카=화학-기계 복합)와
**held-out YAML 자신의 `excluded_axes.abrasive_shape` 반대 주석**(50 nm 쌍에서 제조사와
형상이 동시에 바뀌어 분리 불가 — 항을 만들면 데이터셋 지문이 된다)이 겹쳐 **(B) 배선
보류**로 판정했다. 판정#68의 2회차이고 3회차 미도달이라 종결하지 않는다.

**ρ 를 올리려 배수를 조정하지 않았다** — 코드·YAML 0 변경, C4 여전히 실패.

부수 발견: S(방추형)/N 배수는 고농도에서 0.78~0.94 로 **방향까지 역전**한다 — "비구형이
항상 빠르다"는 그 논문 자신의 결론이 가장 각진 계열(F)에서만 전 구간 성립한다.

**교훈**: 독립 문헌에서 배수를 뽑아냈다는 사실만으로 배선 가능이 보장되지 않는다 —
그 배수가 **대상계 운전점에 대응하는 단일값**인지까지 확인해야 한다. 판정#75의 φ/m 은
"다른 계열에서 나온 값"이었어도 적어도 대상계 안의 값이었다는 점이 대비된다.

**게이트(둘 다 Max워커가 직접 재실행)**: verify_claims ✓ · check_knowledge ✓ ·
pytest **1294 passed / 1 skipped** 회귀 0 · completion **67/70 불변** · qa_loop #300
`--strict` **PASS** 유의 평균 ρ **0.9566 불변** · `git diff sim/ knowledge/params/` 공백.

⚠ 운영: 위임 세션이 ①에서 또 "백그라운드 pytest 대기" 상태로 조기 종료(로그 159 바이트)해
Max워커가 게이트 5종을 직접 돌려 수거했다. ②는 브리프 최상단에 "테스트는 포그라운드로,
Bash timeout 900000ms" 를 박아 **정상 완주**했다 — 이 지시가 실제로 효과가 있다.


## 2026-09-19 [Max워커] 판정#78 — κ 형상축 3회차(최종) 영구 종결 · C4 구조적 미달 확정

판정#68(방향 확정·배선 보류 — 자기채점 회피) → 판정#76(2회차, ma15217525 그래프 판독으로
배수를 처음 확보했으나 농도 의존 비단조 + 대상계 불일치로 전이 불가) 에 이은 3회차이자
마지막이다. EVIDENCE-RULES 3회차 규칙 적용 시점.

**3경로 전수 탐색** (과제가 새로 지정한 미탐색 각도 포함):
1. **로컬 특허 코퍼스 471건 전수 grep** — "fumed vs colloidal 비교실험은 사실상 형상
   대조"라는 각도. "fumed" 165건 중 "colloidal"+"Cu" 동시출현 156건, 근접 상위 6건
   (US8211193B2·US20070254964A1·US20160122590A1·US7118685B1·KR102072230B1·KR101867441B1)
   직접 열람 → **전부 "구형/성형된 고치/집합체" 류 Markush 상투구**이지 실측 비교표 아님.
2. **로컬 논문 코퍼스 1312건 전수** — 형상어+removal rate+Cu 동시출현 17건, 전부
   oxide/사파이어/ZrO2/W 대상이거나 TSV 종횡비 문헌.
3. **OpenAlex 신규 후보 3건** — Armini 2008(JES, Cu 대상계이나 "comparable MRR"로 배수
   없음 + 폴리머 코어라 순수 형상 아님), Gao 2021(Sci. China Mater., 봇차단으로 대상계
   확인 자체 불가 → 반증도 확증도 아닌 **순수 미확인**으로 결론에 미반영), Fang 2017
   (RSC Adv., 연마재가 다이아몬드라 대상계 요건 자체 불충족).

**(B) 영구 종결.** E5/대상계 불충족은 몇 건을 모아도 E3을 만들지 못한다.

**C4 종결 등록은 구조적으로 불가능함을 소스로 확인했다** — `validation/C2-CLOSURES.yaml`
은 `tools/completion.py:274-327` 의 C1/C2(팩터×팩 confidence) 칸만 소비하고, C4 블록
(`tools/completion.py:333-340`, held-out ρ)은 이 파일을 **전혀 참조하지 않는다**. 임의로
스키마를 확장하지 않고, `cu_h2o2_bta` 의 C4 를 **격자상 영구 미충족**으로 확정 보고한다.

> **격자 67/70 이 이 코퍼스 접근 수단의 상한이다.** 남은 3칸은 다른 크론이 작업 중인
> `cu_alkaline_benzenesulfonic` C2 3칸이고, C4 1건은 위 종결로 더 이상 전진 경로가 없다.

코드·YAML 0 변경, MRR 비트 불변. `abrasive_shape` UI 필드(`sim/web/studio3d.html:254`,
`pk:null`)는 죽은 채 유지 — 반쯤 연결하지 않는다(판정#68·#76과 결론 일치).

**교훈 1 — 동시출현 함정**: "새 각도"를 지정받아도 그 각도가 실제로는 특허 명세서의
Markush 상투구일 뿐 실측 비교표가 아닌 경우가 압도적으로 많다. 동시출현 검색만으로
"찾았다"고 판단하면 안 되고, 반드시 상위 후보를 **직접 열람**해 실측표 유무를 확인해야
한다. 판정#76의 "그래프 판독 함정"과 자매 교훈이다.

**교훈 2**: 종결 스키마가 특정 게이트(C2) 전용으로 설계된 경우, 다른 게이트(C4)에 그대로
재사용할 수 있다고 가정하지 말고 소스를 읽어 확인해야 한다 — 안 그러면 존재하지 않는
종결 경로를 있다고 잘못 보고하게 된다.

**부수 — 재현 검증**: sci-hub.ru 캡차 응답이 판정#68이 2026-09-10 에 기록한 서명
(7,363 바이트)과 이번 회차(2026-09-19) 재확인에서 **바이트 단위까지 100 % 일치**했다.
"안 열어봤다"가 아니라 "다시 열어봤는데 똑같이 막혀 있다"를 수치로 확인한 것이다.

**게이트(Max워커 직접 재실행)**: verify_claims ✓(출처 15건 실존 · 검증코드 3블록 통과) ·
check_knowledge ✓ · pytest **1295 passed / 1 skipped** 회귀 0 · completion **67/70 불변**
(C4 그대로 실패 — 숫자로 숨기지 않는다) · qa_loop #303 `--strict` **PASS** 유의 평균 ρ
**0.9566 불변** · `git diff sim/ knowledge/params/` 공백.

⚠ 운영 — **신설 테스트가 즉시 일했다**: 이 회차 중 `tests/test_evidence_ledger_integrity.py`
(같은 날 신설)가 pre-push 클린 트리에서 **실제로 FAIL** 했다. 다른 크론(성장엔진)이 노트에서
...
## 2026-09-20 [Max워커] 판정#80 κ 형상지수 2회차(부분 승격) · 판정#81 χ 산화제 K 2회차(기각)

격자 **68/70 불변**. 두 판정 모두 팩 `cu_alkaline_benzenesulfonic` 의 남은 2칸을 겨눴고,
**한 칸도 오르지 않았다** — 그런데 한쪽은 파라미터 하나를 실제로 승격했다. 칸이 안 오른 것은
κ 가 형상 지수 **두 개 모두** literature 를 요구하기 때문이지 전진이 없었기 때문이 아니다.

**1. 판정#80 — κ `abrasive_conc_exponent` estimated → literature (커밋 d64cd4a, push)**

로컬에 이미 있던 `papers/cooper2002-cu-particle-conc-1517772.pdf`(Cooper et al. 2002,
doi:10.1149/1.1517772) 4쪽 전문을 fitz 로 직접 판독해 3개 확인항목을 모두 충족시켰다:
①"Copper wafers were polished..." — Cu 기판 실측 농도 스윕(Fig.4 Cu 단독, Fig.5 Cu+SiO2)
②연마입자 "silica-based"(알루미나 아님) ③결론 "scale linearly with wt%^1/3 for dielectric
**and metal** substrates" 가 Cu 데이터 자체에 대한 주장. 이전 강등 사유(값 출처
US9499721B2 가 TEOS/oxide 막질 → Cu 로의 **막질 전이**)가 소멸한다 — 전이가 아니라 대상계
직접 실측이 되기 때문이다. 값(0.3333333333)은 불변, source 를 Cooper 2002(1차) +
US9499721B2(보조)로 병기.

⚠ **과제가 묻지 않은 추가 발견**: Cooper 의 Cu 실측은 **산성 + BTA/TA 억제제** 슬러리(Fig.6)로,
이 팩(알칼리 pH 9 · 억제제 없음 · 벤젠술폰산)이 아니라 **부모 팩 cu_h2o2_bta 와 같은 화학계**다 —
원래 강등 사유를 지우자 **더 좁은 불일치가 새로 드러났다**. 그럼에도 승격을 유지한 근거는 둘:
지수 유도가 화학 무관 기하논증이라는 점, 그리고 원문 Fig.6 이 BTA↔TA 억제제 교체에도 지수가
불변임을 **실측으로** 보인다는 점. 결정적으로, 코드베이스가 이미 쓰는 "동일 시스템 = 막질+입자"
정의(oxide_silica 가 같은 값을 literature 로 받은 기준)를 **이 판정에서만 좁혀 새 기준(pH·억제제)을
즉석 발명하지 않았다**.

`abrasive_size_exponent`(0.0)는 **1차 출처 확보 실패** — US6979252B1 단일입경 재확인,
corpus.sqlite 교차검색 2건 무관, find_open_access 로 MDPI ma17194905 를 신규 확보했으나
단일입경·다른 화학으로 부적격. estimated 유지, **3회차 여지 남김**.

**2. 판정#81 — χ `oxidizer_passivation_K` 승격 기각, estimated 유지 (커밋 70e097d, push)**

판정#74(1회차)가 남긴 한계 1(US20110165777A1 TABLE2 **4점**의 잔차 11.9% 가 코퍼스
literature 문턱 ≤7.3% 미달)을 "독립 데이터셋 교차확인"으로 해소하려 했다. 요건은
Cu · 알칼리 pH≥7 · 벤젠술폰산 또는 무억제제 · H2O2 **단독** 스윕 3점 이상 **인쇄값**.

신규 확보·원문 판독 5편 전부 부적격:
- Lin & Du 2009 ECS Trans 18(1):485 (doi:10.1149/1.3096490, Fujimi) — 제목·초록이 과제에
  정확히 부합했으나 Cu RR vs H2O2 가 Fig.1/2 **그래프 전용**, pH 미인쇄, 억제제 성분명 비공개
- Wang A. 2014 J.Semicond 35:026003 — 화학·pH 요건은 **충족**(콜로이달실리카 0.3 vol% ·
  FA/O II 킬레이트 · 무억제제 · pH 10 · H2O2 0~2 vol% 스윕)인데 Fig.1b **그래프 전용**
- Wang C. 2012 J.Semicond 33:116001 — H2O2 3.3 vol% **고정**, 스윕 축이 다른 변수
- Miranda 2004(로컬) — pH 8 고정 시 H2O2 **2점**뿐(3점 미달) + 상용 슬러리라 화학 미확인
- Du 2004(로컬) — pH 4 산성(요건 위반) + 그래프 전용 이중 결격

로컬 특허 코퍼스 471건 재확인도 소득 없음(벤젠술폰산 실사용 1건 = 산성 BTA계 부적격 /
Rohm&Haas·Fujimi 어사이니 196건 → Cu·removal-rate 다출현 37건 압축 → H2O2 wt% 스윕
Example 표 공존 **0건**).

**관찰된 패턴**: 이 하위분야에서 "알칼리 무억제제 Cu" 화학 **자체**는 여러 편 존재한다.
그런데 **H2O2 농도를 단독 축으로 3점 이상 인쇄한 사례가 하나도 없다** — 학술논문은 전부
그래프로만 제시하고 특허는 점수가 모자란다. **교훈: "화학이 맞는 논문을 찾았다"와
"재적합 가능한 인쇄표를 찾았다"는 별개의 성공 기준이다.** 판정#74·#76·#78·#80 이 반복
확인한 "그래프 전용" 패턴이 이번엔 5회 연속으로 나타났다 — 화학 일치만으로 승격을 낙관하면 안 된다.

**종결은 제안하지 않았다** — Cabot·Hitachi·JSR 어사이니 특허군과 Hebei 공업대 그룹
(Wang C./Wang A. 계열) 후속 논문이 미탐색으로 남아 3회차 여지가 있다. YAML·`sim/` **0 변경**.

**3. 부수 — sci-hub 사본 3편 무결성 대조 + INDEX 등록(Max워커 직접)**

위임이 sci-hub.red 로 확보한 3편이 `papers/INDEX.json` 에 **미등록** 상태였다. 세 편 모두
1쪽 제목·저자·권호를 원문과 대조해 오염 사본이 아님을 확인한 뒤(판정#32 의 sci-hub.kr 오염
사례 교훈) 부적격 사유까지 `verdict` 필드에 적어 등록했다 — 다음 회차가 같은 PDF 를 다시
받아 다시 판독하는 것을 막는다. 중복 저장된 동일 파일(`lin2009_h2o2_cu.pdf`, 바이트 동일) 1건 삭제.

**게이트(Max워커 직접 재실행, 두 판정 각각)**: verify_claims ✓ · check_knowledge ✓ ·
pytest **1299 passed / 1 skipped** 회귀 0 · completion **68/70 불변** · qa_loop #309·#310
`--strict` **PASS** 유의 평균 ρ **0.9566 불변**.

⚠ **위임 운영 기록(2회 연속 동일 패턴)**: 두 위임 모두 **pytest 백그라운드 대기 중 조기
종료**했다(로그 각 0바이트·128바이트, max-turns 소진 아님 — 마지막 출력이 "I'll wait for the
pytest background task to complete"). 전체 스위트가 약 10분이라 위임 세션이 그 대기를 버티지
못한다. **다음 회차 개선안**: 브리프에서 위임에게 전체 pytest 를 시키지 말고 관련 테스트
파일만 지정해 돌리게 하고(`pytest tests/test_xxx.py -q`), 전체 스위트·qa_loop·커밋·push 는
Max워커가 수거 단계에서 직접 수행하는 것을 표준 절차로 한다. 이번 회차는 그렇게 처리해
산출물 손실이 없었다.

---

## 2026-09-20 [Max워커] 판정#82 — κ 입경 지수 3회차(최종) 영구 종결, 격자 **68/70 → 69/70**

`abrasive_size_exponent`(0.0, estimated)의 마지막 회차. **값·등급 0 변경**이고 격자가
오른 것은 승격이 아니라 `validation/C2-CLOSURES.yaml` **closed 경로 등록** 때문이다 —
"안 했다"와 "못 한다"를 구분한 결과이지 근거가 생긴 것이 아니다.

**왜 이 칸이 남아 있었나**: 부모 팩(cu_h2o2_bta)의 n=0.0 **null 결과**는 두 다리로
literature 를 받았다 — ①실측(TW202115224A, Cu n=18 구형 부분집합에서 ρ≈0.03~0.15 비유의)
②이론(Chen thesis 힘분배+단층+소성압입 유도가 d⁰). 이 팩(알칼리·억제제 없음)에는 ②만
전이되고 ①은 전이되지 않는다(그 데이터의 조성이 글리신+알라닌+트리아졸). 이번 과제는
**끊긴 ① 다리를 이 팩의 화학에서 복원**하는 것이었다.

**탐색 범위(3회차)**: 로컬 특허 코퍼스 471건 전수 재점수화 · OpenAlex 의 Cooper 2002
(doi:10.1149/1.1517772) **피인용 17건 전수** · `corpus.sqlite` 3종 검색어.

**최근접 후보 2편을 원문까지 직접 판독해 각각 단일 사유로 탈락**(둘 다 sci-hub 확보,
1쪽 제목·저자·소속·권호를 원문 대조해 오염 사본 아님을 확인 후 `papers/INDEX.json` 등록):
- **Lu et al. 2003** *J. Colloid Interface Sci.* 261(1):55 (doi:10.1016/S0021-9797(02)00166-2,
  Clarkson Univ./Matijević 그룹) — Table 7 이 Cu · pH 10 · 입경 100/200/300 nm **3점 인쇄표**로
  요건을 거의 다 충족한다. 그런데 그 슬러리가 5 wt% H2O2 + **1 wt% 글리신**이다 — 배제 대상
  억제제가 혼입돼 ① 다리를 복원하지 못한다(글리신은 애초에 ①이 전이 안 되는 이유였다).
- **Li et al. 2005** *JES* 152(4):G299 (doi:10.1149/1.1869974, Arizona/Fujimi Philipossian
  그룹) — 억제제(BTA/글리신/트리아졸) 언급이 **0건**이라 화학은 맞는데 Table I 입경이
  13/35 nm **2점뿐**(3점 미달)이고 pH 가 원문에 보고되지 않는다.

그 외 부적격을 전부 직접 확인: SiO2/CeO2 core-shell(순수 실리카 아님) · quasi-continuum
시뮬레이션(실측 아님) · HNO3+시트르산+BTA 알루미나개질실리카(산성·BTA·비순수 삼중 위반) ·
"weakly/low-pH alkaline" Cu 슬러리 2편(입경을 **노화 응집 지표**로만 추적, 의도적 스윕 아님) ·
scientific.net 로컬 사본(메타데이터 페이지뿐) · 유기산염/글리신 함침 nano-CRC 2편(억제제 포함).

**결과: 팩 `cu_alkaline_benzenesulfonic` 의 κ 칸은 영구 미충족 확정.** 같은 칸의
`abrasive_conc_exponent` 는 직전 판정#80 으로 literature 승격됐지만 κ 는 지수 **둘 다**
literature 를 요구한다 — 한쪽만 올라서는 칸이 움직이지 않는다는 구조가 이 회차로 확정됐다.

**이번 회차 3판정의 종합 그림**: #80(κ 농도지수 승격) → #81(χ 산화제 K 기각) → #82(κ 입경지수
종결). 세 판정 모두 같은 팩을 겨눴고, **1개 파라미터 승격 + 1개 기각 + 1개 종결**로 갈렸다.
남은 미충족은 **χ 1칸**(`oxidizer_passivation_K` + `oxidizer_peak_wt_pct`, 3회차 여지 있음)과
**C4 1건**(cu_h2o2_bta ρ=0.7175, 판정#78 로 구조적 상한 확정)이다.

**교훈(3회 연속 관측)**: 요건을 "화학 4개 + 관측 1개"로 쪼개 놓으면 후보가 **단일 사유로**
탈락한다 — Lu2003 은 화학 하나(글리신), Li2005 는 관측 하나(2점). 둘을 합치면 요건을 다
덮는데도 각각으로는 못 쓴다. 이것이 "거의 찾았다"가 반복되면서도 칸이 안 오르는 구조이고,
3회차 규칙이 존재하는 이유다.

**게이트(Max워커 직접 재실행)**: verify_claims ✓(출처 **22건** 실존 · 코드 1블록 통과) ·
check_knowledge ✓ · pytest **1299 passed / 1 skipped** 회귀 0 · completion **69/70** ·
qa_loop #311 `--strict` **PASS** 유의 평균 ρ **0.9566 불변**.

⚠ **위임 운영 — 개선이 효과가 있었다**: 이번 브리프는 앞선 2회 연속 실패(위임이 전체 pytest
백그라운드 대기 중 조기종료)를 반영해 **"전체 pytest 를 돌리지 마라, 관련 테스트만 + 커밋까지,
...
### 2026-09-20 — [Max워커] 판정#84 C4 `cu_h2o2_bta` held-out MAPE 폭발 원인 규명

C2가 마감(판정#83)되면서 **C4가 유일한 미완**이 됐다. 그 중 `jani2025_cu_rsm_composition_heldout`
(n=13)의 MAPE **11730.5%**(ρ=+0.302, p=0.158)를 분해했다. 결론은 **(c) 이미 선언된 한계의
정량적 확인 — 새 결함 아님, 코드·YAML 0 변경**.

- **§2 항 분리 실측**: promoter(χ 옥살산 촉진, 판정#75) 항을 떼고 재계산하면
  ρ −0.346 / MAPE **848.2%**, 붙이면 ρ +0.302 / MAPE **11730.5%**. 이 항 하나가 MAPE를
  **13.8배 증폭**시키면서 순위는 옳은 방향으로 뒤집는다 — 전형적 "순위 개선 ↔ 스케일 희생"
  트레이드오프다.
- **§3 근본 원인**: `f(C)=g(C)/g(C_ref)`, `g(x)=φ+(1−φ)(x/C_anchor)^m` 인데 이 팩은
  `promoter_ref_M=0.0`(글리신-only 기준 조성에 옥살산이 없어 Kp 이중계상을 피하려는 정당한
  선택)이다. x=0이면 g(0)=φ라 **분모가 통째로 φ=0.078058**이 되고 (1−φ)/φ=**11.81**이라
  C가 0에서 조금만 벗어나도 배수가 즉시 8배대에서 시작한다(0.02 M에서 이미 8.11배).
  13개 run **전수**가 8.1~20.4배를 맞는다 — 부분집합이 아니라 **구조적**이다.
  φ·m·C_anchor 셋 다 US6309560B1(Cabot, **알루미나**·옥살산암모늄·**BTA 없음**)에서 온
  **타계 전이(E4)** 값인데 구현은 방향뿐 아니라 **크기까지 곱하고** 있다. 다만 이건
  판정#75가 알고 승인한 트레이드오프이고 `sim/chemistry.py:766` 런타임 note가 매 호출마다
  "크기는 순위 목적으로만"을 경고한다. **이번 진단이 새로 정량화한 것**: 기존에 검증된
  것은 "0.02M↔0.08M 비"의 19.2% 오차뿐이고, **C=0 기준 절대 배수(8~20배)의 오차는 한 번도
  검증된 적이 없다**.
- **§4 promoter를 떼도 남는 848%**: 이것도 새 사실이 아니라 같은 팩 캘리브레이션 데이터셋
  (`jani2025_cu_h2o2_acidic_chelator.yaml`) 헤더가 이미 신고한 **baseline ~4.5배 과대**
  (Kp 역산 조성 US20080090500A1의 실험실·툴 vs Jani 벤치탑 CETR-CP-4 질량감량법)다.
  즉 전체 ~95배 과대는 **두 기존 편향의 곱**이다.
- **§5 판정#41 패턴 전수 점검**: held-out 13조건의 `overrides` 10필드가 **전부 명시 선언**
  — 이 데이터셋엔 미전달 축 결함 없음. 단 같은 논문의 **캘리브레이션** 데이터셋에서
  `chelator_M` 미선언 1건을 발견했으나, C와 C_ref가 똑같이 팩 기본값으로 대체돼 배수가
  **항등적으로 1.0**이라 수치 영향 0임을 직접 계산으로 확인했다("선언 누락"은 맞지만
  "조용히 잘못된 값으로 샌다"는 아니다). 선언을 채우면 판정#41의
  `oxidizer_acid_chelator_K`가 암묵 전제한 "이 축은 1.0"이 깨져 재적합이 필요해지므로
  **이번 범위 밖으로 남긴다**.

**ρ를 올리려고 자유 적합하지 않았다** — 두 앵커를 이 held-out으로 재적합하면 자기채점이고,
promoter 축은 E4라 정책상 크기 채택이 "방향만"이어야 한다. **격자 70/70 불변, C4 실패 유지.**
게이트: verify_claims ✓ check_knowledge ✓ test_engine·test_sensitivity 21건 ✓ ·
completion **70/70** 불변 · qa_loop #313 --strict PASS ρ=0.9566 불변. 커밋 c891572.

> **C4를 여는 유일한 경로는 새 독립 유의 held-out 1건이다**(현재 유의 held-out이
> `tw202115224a` 하나뿐이라 평균 = 그 값 0.717). 판정#84 §7이 다음 경로 2개를 구체 지정:
> ①Cu+콜로이달실리카+옥살산+글리신 계에서 **옥살산 C=0 대조점**을 가진 통제쌍 확보(현
> Jani RSM은 0.02~0.08 M만 있어 절대 배수 검증 자체가 불가) ②장비 스케일이 팩 Kp 역산
> 조건과 가까운 Cu DOE.

✅ **위임 운영 표준 복귀**: 이번 브리프는 "전체 pytest 금지(583초) · push 금지 · 관련
...
### 2026-09-20 — [Max워커] 판정#86 `cu_h2o2_bta` baseline Kp ~4.5배 편향 — A·B 둘 다 기각, 원인은 ψ 정의위반

판정#84 §7-2 후속. `jani2025_cu_h2o2_acidic_chelator.yaml` 헤더가 신고해 온 "예측 ~10284 vs
실측 2282 = 4.5배 과대"의 출처를 가설 A(P·V 환산)/B(순수 장비 스케일)로 갈라 보라는 과제였는데
**둘 다 기각됐다**.

- **A 기각 — 방향이 반대다.** Preston 항(P·V·Kp)만 떼면 818.8 nm/min vs 실측 2282 =
  **0.36배 과소**예측이다. "P·V가 부풀렸다"는 가설은 이 조건에서 성립 불가.
- **§2 부수 발견(판정#41류, 그러나 무해)**: 두 데이터셋 **모두** CETR-CP-4의 실제
  `center_offset_m`(r_cc)을 선언하지 않아 팩 기본값 0.200 m(300mm 생산툴용 Zhao 2012)을
  물려받는다. 다만 **앵커(Tugbawa)와 held-out 양쪽에 동일 적용돼 상쇄**되고 V 비는 90/75=
  **1.20배**뿐 — 4.51배를 설명 못 한다. 또 `rpm_wafer=rpm_platen`(Rs=1)이면 Lai 2001 극한식에서
  속도가 웨이퍼 전면 균일 `|v|=ω_p·r_cc`가 되어 **시료 반경이 소거**되므로, 2인치 디스크 vs
  300mm 웨이퍼라는 크기 차이는 V에 들어오지 않는다.
- **B 기각 — 뭉뚱그리면 원인을 가린다.** 장비가 다른 건 사실이나 수치가 나오는 경로는 P·V가
  아니라 **ψ 인자**다.
- **실제 원인**: 이 조건은 BTA 없음(`inhibitor_mM=0`)인데 팩 `inhibitor_ref_mM=1.0` 기준과
  비교되어 **ψ=9.969**가 곱해진다. `sim/factors.py::_f_psi`(1589행)가 이미 런타임마다
  "⚠ ψ>1 — 정의(표면 보호 ≤1) 위반, inhibitor_ref_mM이 범위 하단이 아니라 중간에 있다"고
  **스스로 신고**하고 있고 그 등급은 판정#17·#24가 이미 estimated로 심사한 것이다.
  **새 결함이 아니라 이미 판정된 한계의 정량적 사례 확인**(판정#84가 promoter에 내린 것과 동형).
- **§5·§6**: Jani 원문 대조로 P·rpm·디스크 크기·Table II 값에 **전사 오류 없음** 확인.
  질량감량→MRR 환산은 **코드 경로에 아예 개입하지 않는다**(데이터셋이 논문 인쇄 rate를 직접 사용).
- **고치지 않은 이유**: `inhibitor_ref_mM`을 옮기면 이 팩의 BTA=1mM 캘리브레이션 지점 전부에
  파급되는 독립 결정이다. held-out 하나에 맞춰 옮기면 그게 자기채점이다. 코드·YAML 0 변경.

⚠ **위임 산출물 결함 1건을 테스트가 적발 → Max워커 정정**: 노트가 "후속 판정(원한다면
**다음 미할당 번호** 같은 이름으로)"이라며 **원장에 없는 번호를 선제 인용**해
`tests/test_evidence_ledger_integrity.py::test_no_dangling_evidence_references`가 FAIL 했다.
(⚠ 이 서술 자체가 그 번호를 그대로 적으면 같은 테스트가 다시 FAIL 한다 — 사고를
기록하는 문장은 원인이 된 리터럴을 재현하지 말고 위처럼 풀어 써라. 실제로 재발했다.)
이 테스트는 판정#58이 "원장에 없는 판정을 근거로 쓰는 추적 결함"을 막으려 신설한 것인데
**실제로 작동해 잡았다**. 번호 미할당 표현으로 교체(커밋 dda1d02).
...
- 2026-09-20 오전2: [정확도루프] 판정#88 — `cu_h2o2_bta` 산화제 축 RESPONSE_SPLIT(score 110) 처리. US8501625B2 6점에서 압력비가 H2O2 농도에 의존(1.909/1.846/**4.316**, 산포 2.338배)함을 확인 → 곱셈분리형 `f(P)·g(C)` 구조 반증. Paul 2001(doi:10.1149/1.1372222, 원문 신규 확보) Eq.18 직렬형과 두 단계 LH형은 **압력 2배에 대해 항상 2배 미만**이라는 해석적 상한을 가지므로 4.316배를 원리적으로 재현 불가 — 후보 함수형 계열 전체를 반증. 임계압 P0 = −0.100/−0.182/**+0.698** psi(15/9/3 wt%). 배선은 자기채점·자유도 부족·형상 재현 실패(강제적합 MAPE 20.6%인데 1 psi 정점 못 맞힘)로 보류, `sim/` 0변경. 노트 `knowledge/cmp/pressure-oxidizer-interaction-series-model-falsification.md`. pytest **1304 passed**/1 skipped, qa_loop #321 --strict PASS ρ 0.9566 불변, completion 70/70 불변. ⚠ 다음 회차는 이 갭을 `--skip "산화제 농도"`로 넘겨라(판정#88로 종결, 재개 조건 = 농도 3수준×압력 3수준 이상 독립 DOE).
- 2026-09-20 정오: [정확도루프] 판정#92 — C4 `cu_h2o2_bta` 갭에 새 held-out 1건 등록 (`validation/datasets/ihnfeldt2008_cu_alumina_ph_oxidizer_chelator.yaml`, n=7). 코퍼스 전문 1,661편 전수 스캔으로 Ihnfeldt 2008 UCSD 박사논문 Table 6.2(6조성×3pH 인쇄 MRR)를 발굴, 학술지판 doi:10.1149/1.2912977 을 신규 확보해 공정조건을 원문 문구 assert 로 재확인. 실측 ρ=−0.143 p=0.6435 (비유의 → 유의 평균 ρ 0.9566 불변, C4 격자 불변). **핵심 산출 = 구조 반증**: 억제형 g(C)=φ+(1−φ)(1−θ)/(1−θ_ref) 는 모든 K·φ 에서 f(0.1)/f(0) ≤ 1 인데 관측은 pH 10 에서 H₂O₂ 0→0.1 wt% 에 15→350 nm/min = **23.33배 증가** → φ 로 부호를 만들 수 없음이 확정(스윕 0.9239~0.9788). 산화제 항은 단봉형(저농도 촉진·고농도 억제)이고 정점이 pH 의존. 배선은 축퇴(판정#19)·자기채점(판정#75)·대체 함수형 근거 미확보로 보류, `sim/` 0변경. 노트 `knowledge/cmp/chi-cu-oxidizer-zero-point-ihnfeldt-falsification.md`. qa_loop #323 --strict PASS, 감사 clean. 다음 회차 요건 = 같은 pH·같은 조성에서 H₂O₂ 0 포함 4수준 이상 인쇄표.
- 2026-09-20 오후: [Max워커] 판정#91·#93 — C4 미달의 성격을 '모델 결함'에서 '데이터 한계'로 기계 판별. `tools/rank_ceiling.py` 신설: held-out ρ의 **구조적 상한**(모델이 보는 고유 입력 그룹 수로 결정)을 `backtest`의 실제 `_recipe_from`+`simulate` 경로로 계산. **핵심 실측** — `tw202115224a`는 `abrasive_size_exponent=0.0`이라 입경 채널이 통째로 불활성이고 살아있는 축은 압력뿐이라 **고유 예측 2종**, 상한 0.71745 = **현재 실제 ρ와 완전 일치(달성률 100%)**. 즉 C4 미달은 모델이 덜 좋아서가 아니라 이 held-out이 더 줄 정보가 없어서다. 상한<1.0인 구조적 제약 4건 식별(`miranda2004`는 상한 0.0). 이어 판정#93에서 **C4 적격 필터**(범위 안·비캘리브레이션·유의)를 `completion.heldout_by_pack`과 소스 공유로 적용 — 그 전엔 순위 최상위 4건이 전부 C4에 기여 못 하는 데이터셋이라 작업자를 무의미한 곳으로 보내고 있었다(판정#58·판정 ㊊에 이은 **세 번째 동형 결함**: 게이트 필터를 진단 도구가 공유 안 함). `--c4-requirement` 로 도달가능성 산술 추가(k=1 → ρ≥0.98255, 판정#90 독립 유도값과 일치). **C4 기준·격자 로직 미변경 — 기준을 낮춰 통과시키지 않았다**: 격자 70/70 불변, C4 1건 실패 그대로. `sim/`·팩 YAML·데이터셋 0 변경(MRR 비트 불변). pytest 1318 passed(#91 시점 전체 스위트) / 35 passed(#93 대상), qa_loop #325 --strict PASS ρ=0.9566 불변. ⚠ 위임 2건 모두 게이트 전 조기종료 → 결함 3건 적발·정정·원장·노트를 Max워커가 직접 수행.
- 2026-09-20 오후: [정확도루프] 판정#94 — 갭 랭커 최상위 `RESPONSE_CONFLICT`(cu_h2o2_bta/pH, score 120)가 **채점 도구 결함**임을 확인, `sim/` 수치 0변경. `_ph_cu_acidic_term`은 관측 없는 레짐(알칼리×억제제有)에서 항을 끄는데(판정#34 설계) 이 팩은 기본 BTA 1mM이라 pH>6.25 전 구간이 침묵이다 — 엔진 실측으로 pH 7/8.3/10/11 MRR이 500.5 nm/min로 완전 동일(차<1e-9)함을 확인. `response_map`이 그 침묵을 상수 예측으로 읽어 감소하는 산성 가지와 이어 **인공 골짜기**를 만들었고, 문헌 정점 3점 중 2점(pH 8.3·10)이 침묵 구간 안에 있었다. 수정: `Factor.gated`(구조화된 포기 사유) 신설 + `sweep()`이 침묵 점과 **문헌 점을 같이** 잘라냄 + 새 판정 GATED(score 80, CONFLICT 120보다 낮음 — 처방이 '코드 수정'이 아니라 '데이터 확보'다). 전 팩 CONFLICT 1→0. 회귀 8건 신설. pytest 1348 passed/1 skipped, completion **70/70 불변**, qa_loop #326 --strict PASS ρ **0.9566 불변**. 노트 `knowledge/cmp/response-map-gated-regime-silence-vs-prediction.md`.
- 2026-09-20 저녁: [Max워커] 판정#95 — `tools/model_hygiene.py` 극한(L) 검사의 **조용한 건너뜀** 제거. 21건 중 **11건이 "극한값으로 바꾸지 못함"(warn)** 으로 끝나 검사가 **수행되지 않았는데 통과처럼 읽히고** 있었다(`time_s` 7팩 = Recipe 필드라 YAML 정규식 치환이 원리적으로 실패 / `dispersant_type` 4팩 = 문자열 범주형이라 숫자 정규식 미매칭). `dataclasses.fields(Recipe)` introspect 주입 + `CATEGORICAL_ABSENT`(범주형 '없음'을 **모델이 선언**) + 잔여는 **warn→error 격상** + `driver_seen` 판정("항이 살아서 낸 1.0"과 "항이 꺼지며 남은 1.0"은 정반대 정보다)으로 **11건→0건**. **부수 발견이 본 수확**: (A) τ의 `time_s=0`은 `_f_tau`의 `t_pol>0` 가드로 항이 스스로 꺼진 것인데 검사기가 MODULATOR 통과로 오독 — τ 물리는 **미변경**, 대신 τ(1 s)=0.05211867631070653이 Philipossian 2004(doi:10.1149/1.1731539)·Mu 2016(doi:10.1016/j.mee.2016.02.035) **인쇄값 5개로 완전히 결정**됨을 폐형식 재도출(a를 역산해 두 실측점 오차 0.0001 nm 이내 재현, 팩 선언값과 0.04% 일치). (B) **`chelator_M=0`에서 ψ=1.223>1이 어느 게이트에도 안 걸리던 사각지대** — `_f_psi`의 ψ>1 정의위반 검사가 착화제 항 곱셈 **전의** v에만 걸려 있었다. 최종 v로 이동(**미클램프**, 신고만). `LIMIT_ROLE` 미선언 2건(`chelator_M`·`promoter_M`) MODULATOR 등록. **판정#58·㊊·#93에 이은 동형 4번째**라 일반 규칙 승격: "진단 도구는 **아무것도 검사하지 못한 경우**의 출력을 반드시 설계하고 통과와 다른 등급으로 낼 것". 되돌림을 실제 실행해 FAIL 재현 확인. `sim/` 수치·YAML 0변경, MRR 비트 불변. 게이트(Max워커 직접 실행): pytest **1366 passed/1 skipped 회귀 0**(1318→+48) · completion **70/70 불변** · qa_loop #327 --strict PASS ρ=0.9566 불변 · verify_claims ✓ check_knowledge ✓. 커밋 e946159, push 완료
- 2026-09-20 저녁2: [Max워커] 판정#96 — `model_hygiene.py` 차원(D) 검사의 **중위 무차원 규칙 과잉적용** 제거. 판정#95 직후 **같은 도구 두 번째 결함**: 매 회차 **8건**(`cu_ph_acid_k` 등 1/pH 6건 + `sic_kmno4_ph_floor_*_wt` wt% 2건)을 "무차원이어야 하는데 단위가 있다"로 신고했는데 **전부 오탐**이었다. `check_dimensions()`가 `DIMLESS_SUFFIX` 튜플 **전체**를 중위로도 적용해 `_ph`가 "이름에 pH가 들어가지만 그 자신은 무차원이 아닌" 키까지 집어삼킨 것 — 중위 도입의 본래 의도는 `promoter_exponent_m`처럼 지수를 가리키는 한 글자가 뒤에 붙는 경우 **하나뿐**이었다. 원문 대조로 8건 정당성 확인(`exp(-k·(pH-ph_ref))`의 k는 지수 인자가 무차원이려면 **1/pH가 맞고**, φ 농도 보간 앵커는 **wt%가 맞다**). **정량 재현 2건**: `cu_ph_acid_k`=0.1428이 US20080090500A1 TABLE 4 계열별 k 0.1360/0.1496/0.1427(폭 10.0%)의 pooled 적합 범위 안·중앙계열과 0.1% 이내 / `w_ph_acid_k`=0.1163이 Stojadinović 2016(doi:10.1007/s40735-016-0041-4) Table 1 MRR비 1.429/1.533/1.300 로그평균÷ΔpH=3에서 **5e-4 이내 재계산**. **새 판정 기준(물질명 없는 구조 규칙)**: 그 표지가 **키가 가리키는 양 자체**를 무차원으로 만드는가(지수) vs **어떤 축에 대한 양**인지를 말할 뿐인가(pH축 감쇠계수=1/pH) — 후자는 중위 표지 불가. `DIMLESS_INFIX=("_exponent",)`로 수축. **수축의 반대방향 위험도 검사**(면제 잃은 키 12개 전수 — 전부 단위 정당 또는 note 예외 인정, 검사가 꺼진 게 아님). 회귀 4건, **되돌림 실제 실행해 2건 FAIL 확인 후 복원**. `sim/`·YAML **0 변경**(값이 아니라 검사기가 틀렸다), 차원 신고 **8→0건**, 도구 전체 **17→9건**. **교훈: 검사기의 오탐은 사소한 노이즈가 아니라 진짜 결함을 가리는 차폐물이다** — 이 8건은 판정#95가 고친 극한(L) 11건과 같은 출력에 나란히 찍히며 서로를 "어차피 늘 뜨는 것"으로 정당화하고 있었다. 게이트: pytest **1370 passed/1 skipped 회귀 0** · completion **70/70 불변** · qa_loop #328 --strict PASS ρ=0.9566 불변. 커밋 20d0a9a, push 완료
