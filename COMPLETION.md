...
| 우선 | 항목 | 사유 | 기한 |
|---|---|---|---|
| 1 | **C4 sic_ceria_h2o2 유의 held-out ≥1** | 유일하게 "데이터가 없으면 못 푸는" 조건. 나머지는 문헌 작업이라 시간만 있으면 되지만 이건 막히면 기한 전체가 무너진다. 현재 sic2026 DOE50 ρ=0.41(p=0.002지만 MAPE 2736%)·entegris ρ=-1.0 → SiC 추가 데이터셋 확보 or 스코프 조정 | 09-14 |
| 2 | **C2 Λ·Θ·κ (15칸)** | 전 팩 공통이라 노트 1편이 5칸을 올린다. Λ=용해/전기화학, Θ=열수지, κ=입경-접촉 | 09-17 |
| 3 | **C1 ψ 3칸 + Δ 3칸** | 경로는 위 "축별 완성 경로"에 이미 확정. 구현만 남음 | 09-18 |
| 4 | **C2 Γ·χ·ψ·τ·Δ·S 잔여 23칸** | 개별 팩 근거 필요분 | 09-20 |
| 5 | **C5 5축 verify 노트 + C8 MODEL-BASIS.md 최종** | Π·ψ·τ·Δ·S | 09-21 |
