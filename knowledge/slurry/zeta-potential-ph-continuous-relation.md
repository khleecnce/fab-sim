...
> **⚠ 2026-09-18 추기 — 독립 중복 조사가 있었다.** 같은 날 다른 세션이 같은 질문을
> **다른 문헌**(Gong 2024 doi:10.3390/ma17030679 · Dawkins 2019 doi:10.7939/r3-g3c2-xe63)으로
> 독립 조사해 [[../cmp/zeta-potential-ph-continuous-relation]] 에 정리했다.
> 그 노트 §8 에 두 결과의 **교차대조표와 근거서열 판정**이 있다. 요점:
> - **알루미나 IEP**: 이 노트의 Gopal & Talbot 2006 이 **지지전해질 1 mM KNO3 를 명시**해
>   이온세기 통제 실측(E1)이고, 상대 노트의 Gong 2024(8.72)는 이온세기 미기재(E3) →
>   **E1 > E3 으로 이 노트의 IEP=9.0 이 채택**됐다(Gong 은 3.1% 이내 보강 증거).
>   두 값을 평균낸 수는 만들지 않았다.
> - **세리아 IEP**: 이 노트 ~8 vs 상대 9.6~9.8 로 약 20% 갈리나, 두 문헌의 세리아가
>   같은 물질이 아니다(pure ceria vs Nyacol 상용 콜로이드). 이 노트 §2 가 이미 적은
>   "전처리·하소·입경 편차" 해석이 **독립 데이터로 지지됐다** — 세리아 IEP 는 단일 상수가
>   아니라 제품별 실측값이라는 것이 두 세션의 공통 결론이다. 팩값(6.8)은 양쪽 다 안 고쳤다.
> - **세리아 기울기**는 -16.7(이 노트) vs -13.9(상대) 로 **오더 정합**(구간 정의 차이로 설명 가능).
> - 결론 3가지(조건 불일치로 절대 mV 대입 불가 / 재사용 가능한 건 부호·IEP·기울기 오더 /
>   `boltzmann_surface_enrichment` 대체는 이온세기 보정이 선결)는 **두 세션이 독립적으로
>   같은 곳에 도달**했다.
