...
## 7-B. 후속(판정#93) — 도구가 "고치면 판정이 움직이는 곳"을 가리키게

§4 표를 `--all` 로 뽑으면 **상한은 1.0에 가까운데 달성률이 바닥인** 네 건이
가장 눈에 띈다. 그런데 넷 다 C4 집계에 **한 칸도 기여하지 않는다**:

| 데이터셋 | 상한 | 달성률 | in_scope | used_for_calibration | C4 적격 |
|---|---|---|---|---|---|
| yang2023_quartz_ceria_L25 | 0.9996 | -0.3% | **false** | false | ✗ 범위밖 |
| gong2024_4hsic_alumina_kmno4_L25 | 0.9909 | 6.5% | true | **true** | ✗ 캘리브레이션 |
| carbide2023_slurry_composition_L9 | 1.0000 | 16.7% | **false** | false | ✗ 범위밖 |
| mo2026_double_sided_L16 | 0.9983 | 17.9% | **false** | false | ✗ 범위밖 |

`tools/completion.py::heldout_by_pack` 은 `in_scope and not used_for_calibration
and significant` 인 것만 센다. 즉 이 순위표를 보고 yang2023(석영유리 — 팩
재료계 밖)의 ρ를 올리려 물리를 건드리면 **C4는 안 움직이고 모델만 오염된다.**

이것은 새 유형이 아니라 **세 번째 동형 결함**이다:

1. 판정#58 — `accuracy_gaps.gaps_bias()` 가 `used_for_calibration` 필터를
   빠뜨려 자기 캘리브레이션 데이터를 재심사하는 순환
2. 판정 ㊊(커밋 7a11f2b) — `blockers.py` 가 영구 종결 칸을 1위로 올려 4회차를 유도
3. 이번 — `rank_ceiling.py` 가 C4 비적격 데이터셋을 최상위로

셋 다 원인이 같다: **판정 게이트가 쓰는 필터를 진단 도구가 공유하지 않는 것.**
그래서 이번 수정은 조건을 복붙하지 않고 `completion.heldout_by_pack` 과 같은
소스를 공유하도록 했고, 한쪽만 바뀌면 FAIL 하는 테스트로 고정했다.

### 7-B-1. C4 도달가능성 산술 (`--c4-requirement cu_h2o2_bta`)

C4는 유의 held-out의 **평균** ρ ≥ RHO_MIN 이므로, k건을 새로 추가할 때

    sum_new >= RHO_MIN * (n_sig + k) - existing_sum

현재 `cu_h2o2_bta` 는 n_sig=1, existing_sum=0.71745(tw202115224a, §3에서 이미
구조적 상한에 고정 — 더 오를 수 없다). 따라서:

- **k=1**: 새 held-out의 ρ ≥ **0.98255**
- **k=2**: 두 ρ의 합 ≥ **1.83255** (평균 0.91628)

여기에 **순열검정 p<0.05 를 동시에** 넘어야 한다. n이 작으면 ρ가 이산적이라
두 조건이 양립 불가일 수 있으므로, 후보마다 그 n에서 유의해지는 최소 ρ를
`backtest.perm_p_value` 로 실제 계산했다(n=6 → ρ≥0.8286 정확 / n=13 →
ρ≥0.4725 근사 / n=7 → ρ≥0.7143 정확).

판정 결과 — `hong2007`(상한 0.6708, 상한p=0.1167)과 `miranda2004`(상한 0.0,
상한p=0.5417)는 **상한 자체가 영원히 비유의**라 어떤 물리 개선으로도 C4에
기여할 수 없다. 실현가능 후보는 **3건**: `ihnfeldt2008`·`jani2025_rsm`·`us8501625b2`.

> 판정#90이 독립적으로 유도한 "새 held-out ρ≥0.9826 필요"와 이번 재계산
> 0.98255 가 일치한다 — 서로 다른 경로의 교차검증이다.

### 7-B-2. ⚠ 위임 산출물의 결함 1건 (호출자 적발·정정)

k=2 의 **개별 후보 판정**이 자기 상한(최대 1.0)을 **합** 요구치(1.8325)와 직접
비교했다. Spearman 상한은 1.0 이므로 요구 합이 1을 넘는 순간 **모든 후보가
자동으로 '불가능'** 으로 찍힌다 — 실제로는 둘이 합쳐 넘기면 되는데도.
그 결과 바로 아래 쌍별 절이 같은 셋을 '가능'으로 옳게 판정해, **한 출력 안에서
두 절이 정면으로 모순**됐다.

올바른 개별 판정은 "이 후보가 들어가는 실현가능한 k-조합이 하나라도 있는가"
= 자기 상한 + (다른 후보 상한 중 큰 k-1개의 합) ≥ 요구 합. 정정 후 k=2 의
가능 후보가 0건 → 3건이 되고 쌍별 절과 일치한다.

회귀는 **두 절의 무모순** 자체를 검사해 고정했다(값을 몰라도 결함을 잡는다).
정정을 되돌리면 실제로 2건이 FAIL 함을 실행으로 확인했다 — §5 에 적은
"상대적 성질만 검사하면 21,600km 해도 통과한다"는 교훈의 적용이다.
